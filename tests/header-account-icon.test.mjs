...
test("the always-present hamburger drawer owns account destinations while language stays beside it", async () => {
  ...
  assert.equal((header.match(/<HeaderAccount lang=\{lang\}/g) ?? []).length, 1);
  assert.match(header, /<HeaderAccount lang=\{lang\} mobile onNavigate=/);
  assert.match(header, /className="header-controls"[\s\S]*?<InterfaceLanguageMenu lang=\{lang\}\/?>[\s\S]*?className=\{`hamburger-button/);
