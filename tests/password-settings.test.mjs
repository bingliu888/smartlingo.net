...
test("account supports setting and updating a Clerk password", async () => {
  ...
  const route = await readFile(new URL("../app/api/account/password/route.ts", import.meta.url), "utf8");
  assert.match(form, /fetch\("\/api\/account\/password"/);
  assert.match(form, /user\?\.passwordEnabled/);
  assert.match(form, /passwordSettingsMode\(passwordEnabled\)/);
  assert.doesNotMatch(form, /user\?\.updatePassword|useReverification/);
  assert.match(form, /currentPassword: mode\.requiresCurrentPassword \? currentPassword : undefined/);
  assert.match(form, /Enter your current password/);
  assert.match(form, /请输入当前密码/);
  ...
  assert.match(passwordInput, /data-layout-allow-overlap="intentional"/);
