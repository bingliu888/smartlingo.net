...
export function LanguageSubscriptionCatalog({ lang, language }: { lang: SiteLanguage; language: SmartLingoCommunityLanguage }) {
  ...
  return <section className="sl-package-catalog" data-layout-fill="course-packages">
    ...
    <section className="sl-crypto-cta" aria-labelledby="sl-crypto-heading"><div><p className="section-kicker">CRYPTO PAYMENT</p><h2 id="sl-crypto-heading">{t("Pay with crypto", "使用加密货币付款")}</h2><p>{t("Your learning language is already selected. Continue to choose only the three-month course products and token combinations currently enabled in this site's SmartPay5 contract, then connect one wallet to pay.", "学习语言已经选好。下一步只会显示本站 SmartPay5 合约当前已启用的三个月课程与代币组合，然后连接一个钱包完成付款。")}</p></div><Link className="primary-button" href={`/${lang}/programs/${language}/pay/crypto`}>{t("Use crypto payment", "使用加密货币付款")} <span>→</span></Link></section>
