...
export function LanguageSubscriptionCatalog({ lang, language }: { lang: SiteLanguage; language: SmartLingoCommunityLanguage }) {
  ...
  return <section className="sl-package-catalog" data-layout-fill="course-levels">
    <header>
      ...
      <p>{t("Our free five-skill placement recommends Beginner, Intermediate, or Advanced. You can change the recommendation. Taking the placement or viewing a course never starts your Max trial.", "免费五项技能测评会推荐初级、中级或高级；您也可以改选。参加测评或查看课程都不会启动旗舰版试用。")}</p>
    </header>
    ...
    <section className="sl-max-cta" aria-labelledby="sl-max-heading">
      <div><p className="section-kicker">SMARTLINGO MAX</p><h2 id="sl-max-heading">{t("Beginner free. Max for higher levels.", "初级免费，旗舰版学习更高等级。")}</h2><p>{t("Learn Beginner free with ads. A 7-day Max trial starts only if you explicitly choose to begin an Intermediate or Advanced course. After it ends, choose Max for ad-free higher-level access. Max is $119 for 6 months or $199 annually.", "初级课程可带广告免费学习。仅当您明确选择开始中级或高级课程时，才会启动一次 7 天旗舰版试用；试用结束后选择旗舰版，即可无广告继续更高等级。旗舰版为 6 个月 $119 或年度 $199。")}</p></div>
      <Link className="primary-button" href={`/${lang}/pricing`}>{t("See Free and Max", "查看免费与旗舰版")} →</Link>
    </section>
