...
type Card = {
  ...
  sceneKey?: string; options?: Option[]; direction?: "ltr" | "rtl"; difficulty?: number; frequencyDegree?: number; gradeLevel?: number;
};
type LibraryItem = { id: string; form: string; targetPhonetic: string; meaningEn: string; meaningZh: string; sceneKey: string; direction: "ltr" | "rtl"; difficulty?: number; frequencyDegree?: number; gradeLevel?: number };
type TrialPayload = { localDate: string; summary: { total: number }; dailyDeck: Card[]; items: LibraryItem[]; error?: string };
type Task = { taskId: string; skill: Skill; prompt: string; context?: string; audioText?: string; options?: readonly { id: string; label: string }[]; sentenceExercises?: readonly { id: string; scenario: string; prompt: string; audioText?: string; answerTokens: readonly string[]; choiceTokens?: readonly string[]; sourceLanguage?: string; answerLanguage?: string }[]; direction?: "ltr" | "rtl" };
  ...
  return <section className="trial-shell" data-layout-fill="anonymous-trial" data-layout-ready="true" data-layout-overlap-check="anonymous-trial">
    ...
    <article className="trial-activity" style={{ "--trial-accent": ACCENTS[active] } as CSSProperties}><header><div><span>{String(SKILLS.indexOf(active) + 1).padStart(2, "0")}</span><h2>{zh ? labels[active][0] : labels[active][1]}</h2></div>{active === "vocabulary" ? <a href="#trial-vocabulary-report">{zh ? "查看词汇报告" : "View vocabulary report"} ↓</a> : null}</header>
      {active === "vocabulary" && card ? <div className={`trial-card tries-${tries}`} dir={card.direction || direction}><div className="trial-progress"><span style={{ width: `${Math.round((index + 1) * 100 / Math.max(1, cards.length))}%` }}/></div><p>{index + 1} / {cards.length} · {zh ? `Beginner 共 ${total.toLocaleString()} 词` : `${total.toLocaleString()} Beginner words`}</p>
        <button className="trial-flip" type="button" onClick={phase === "study" ? () => { setRevealed(true); setPhase("answer"); } : undefined}><span>{SCENES[card.sceneKey || ""] || "✨"}</span><strong>{revealed ? meaning : card.form}</strong>{!revealed ? <em>{card.targetPhonetic || card.pronunciation}</em> : <em>{zh ? `难度 ${card.difficulty || 1}/5 · 常用度 ${card.frequencyDegree || 10}/10` : `Difficulty ${card.difficulty || 1}/5 · Frequency ${card.frequencyDegree || 10}/10`} · {vocabularyGradeLabel(card.gradeLevel, zh ? "zh" : "en")}</em>}<small>{revealed ? (zh ? "已查看释义，请回答下面的问题" : "Meaning revealed. Answer below.") : (zh ? "点一下查看意思" : "Tap to see the meaning")}</small></button>
