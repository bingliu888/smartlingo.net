...
export function AnonymousBeginnerTrial({ lang, language, languageName, speechLocale, direction, cards: seedCards, tasks, initialSkill = "vocabulary", lockedSkill = false, classId }: {
  ...
  return <section className="trial-shell" data-layout-fill="anonymous-trial" data-layout-ready="true" data-layout-overlap-check="anonymous-trial">
    ...
    <section className="trial-cta"><div><p>{zh ? "喜欢这个学习方式？" : "Like this way of learning?"}</p><h2>{zh ? "免费注册并保存学习进度。" : "Create a free account and save your progress."}</h2><span>{zh ? "登录后会保存进度、分数、21 天词汇记忆和证书记录；全部初级课程永久免费，首次进入中级或高级时会开始一次 7 天 Max 试用。" : "Sign in to save progress, scores, 21-day vocabulary memory, and certificate records. Every Beginner course is free; entering Intermediate or Advanced starts one 7-day Max trial."}</span></div><nav><Link href={`/${lang}/auth/sign-up?returnTo=${encodeURIComponent(`/${lang}/programs/${language}`)}`}>{zh ? "免费注册" : "Create free account"} →</Link><Link href={`/${lang}/play?language=${language}`}>{zh ? "返回边玩边学" : "Back to Play"}</Link></nav></section>
