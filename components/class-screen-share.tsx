...
import { initRealtimeKitClient } from "@/lib/realtime-browser-compat";
...
export function ClassScreenShareButton({ client, manager, lang, disabled, onError, ensurePublisher, onSharingChange }: {
  ...
  useEffect(()=>{const timer=window.setTimeout(()=>{const nav=navigator as Navigator&{platform?:string;maxTouchPoints?:number};const mobile=/Android|iPhone|iPad|iPod/i.test(nav.userAgent)||(nav.platform==="MacIntel"&&(nav.maxTouchPoints||0)>1);setSupported(!mobile&&Boolean(navigator.mediaDevices?.getDisplayMedia))},0);return()=>window.clearTimeout(timer)},[]);
  useEffect(() => {
    if(!client){const timer=window.setTimeout(()=>setSharing(false),0);return()=>window.clearTimeout(timer);}
...
export function ClassScreenShareStage({ client, lang, listening }: { client: RTKClient; lang: Lang; listening: boolean }) {
  ...
  const activeFullId = fullId && shares.some((share) => share.id === fullId) ? fullId : null;
  if (!shares.length) return null;
  const full = shares.find((share) => share.id === activeFullId);
  return <><section className="class-screen-share-stage"><header><strong>{lang === "zh" ? "正在共享屏幕" : "Screen sharing"}</strong><span>{shares.length}</span></header>{shares.map((share) => <Player key={share.id} share={share} listening={listening && share.id !== activeFullId} onOpen={share.local?undefined:()=>setFullId(share.id)}/>)}</section>{full && typeof document !== "undefined" ? createPortal(<div className="class-screen-share-fullscreen" onClick={() => setFullId(null)} role="dialog" aria-modal="true"><Player share={full} listening={listening} full/><button onClick={() => setFullId(null)} aria-label={lang === "zh" ? "返回课程" : "Back to course"}>×</button></div>, document.body) : null}</>;
}
...
export function ClassAudioScreenShare({ code, displayName, manager, parentSessionToken, lang, listening, onError, apiBase="/api/classrooms" }: { code:string; displayName:string; manager:boolean; parentSessionToken:string; lang:Lang; listening:boolean; onError:(message:string)=>void; apiBase?:string }) {
  const [client,setClient]=useState<RTKClient>();const[active,setActive]=useState(false);const[camera,setCamera]=useState(false);const[contentSource,setContentSource]=useState<"file"|"web"|"whiteboard">();const[localTrack,setLocalTrack]=useState<MediaStreamTrack>();const[target,setTarget]=useState<Element|null>(null);const clientRef=useRef<RTKClient|undefined>(undefined);const joiningRef=useRef<Promise<RTKClient|undefined>|undefined>(undefined);const sharingRef=useRef(false);const cameraRef=useRef(false);const localTrackRef=useRef<MediaStreamTrack|undefined>(undefined);const companionTokenRef=useRef("");const companionIdentityRef=useRef("");
  const signal=useCallback(async(value:boolean)=>{if(!manager)return;await fetch(`${apiBase}/${code}/media`,{method:"POST",headers:{"content-type":"application/json"},body:JSON.stringify({action:"screen-share",value,sessionToken:companionTokenRef.current||parentSessionToken,identity:companionIdentityRef.current||undefined}),keepalive:!value}).catch(()=>undefined);setActive(value)},[apiBase,code,manager,parentSessionToken]);
  ...
  const leave=useCallback(async()=>{const current=clientRef.current,token=companionTokenRef.current,identity=companionIdentityRef.current;clientRef.current=undefined;companionTokenRef.current="";companionIdentityRef.current="";setClient(undefined);releaseTrack();if(token)await fetch(`${apiBase}/${code}/media`,{method:"POST",headers:{"content-type":"application/json"},body:JSON.stringify({action:"release-companion",sessionToken:token,identity}),keepalive:true}).catch(()=>undefined);if(current)await current.leave().catch(()=>undefined)},[apiBase,code,releaseTrack]);
  const connect=useCallback(async()=>{if(clientRef.current?.self.roomJoined)return clientRef.current;if(joiningRef.current)return joiningRef.current;joiningRef.current=(async()=>{const response=await fetch(`${apiBase}/${code}/join`,{method:"POST",headers:{"content-type":"application/json"},body:JSON.stringify({displayName,password:(()=>{try{return String(JSON.parse(sessionStorage.getItem(`class-entry-${code}`)||"{}").password||"")}catch{return""}})(),screenShareCompanion:true,sessionToken:parentSessionToken})});const data=await response.json().catch(()=>({}))as{authToken?:string;sessionToken?:string;identity?:string;error?:string};if(!response.ok||!data.authToken||!data.sessionToken||!data.identity)throw new Error(data.error||"SCREEN_SHARE_COMPANION_FAILED");companionTokenRef.current=data.sessionToken;companionIdentityRef.current=data.identity;const next=await initRealtimeKitClient({authToken:data.authToken,defaults:{audio:false,video:false}});await next.join();if(manager)await ensureCompanionStage(next);await next.self.disableAudio().catch(()=>undefined);await next.self.disableVideo().catch(()=>undefined);clientRef.current=next;setClient(next);return next})().catch(issue=>{onError(issue instanceof Error?issue.message:"Unable to join screen sharing");return undefined}).finally(()=>{joiningRef.current=undefined});return joiningRef.current},[apiBase,code,displayName,manager,onError,parentSessionToken]);
  useEffect(()=>{const timer=window.setTimeout(()=>setTarget(document.querySelector(".class-room-controls")?.parentElement||document.body),0);return()=>window.clearTimeout(timer)},[]);
...
function ClassCompanionCameraStage({client,lang,localTrack}:{client:RTKClient;lang:Lang;localTrack?:MediaStreamTrack}){
  ...
  useEffect(()=>{
    type EventMap={on:(event:string,listener:(peer:{id?:string})=>void)=>void;off:(event:string,listener:(peer:{id?:string})=>void)=>void};
    const maps=[client.participants.all,client.participants.joined,client.participants.active,client.participants.videoSubscribed]as unknown as EventMap[];
    const refresh=()=>setRevision(value=>value+1);
    const departed=(peer:{id?:string})=>{if(peer.id)subscribedRef.current.delete(peer.id);refresh()};
    for(const map of maps){map.on("participantJoined",refresh);map.on("participantLeft",departed);map.on("participantsUpdate",refresh);map.on("videoUpdate",refresh)}
    return()=>{for(const map of maps){map.off("participantJoined",refresh);map.off("participantLeft",departed);map.off("participantsUpdate",refresh);map.off("videoUpdate",refresh)}};
  },[client]);
  useEffect(()=>{
    let cancelled=false;const timers:number[]=[];
    const discover=async()=>{if(document.hidden)return;try{const peers=await client.participants.getAllJoinedPeers("",100,0)as unknown as ScreenPeer[];if(cancelled)return;const remote=peers.filter(peer=>peer.id!==client.self.id);setDiscovered(current=>current.length===remote.length&&current.every((peer,index)=>peer.id===remote[index]?.id)?current:remote);const missing=remote.map(peer=>peer.id).filter(id=>!subscribedRef.current.has(id));if(missing.length){missing.forEach(id=>subscribedRef.current.add(id));await client.participants.subscribe(missing,["video"]).catch(()=>missing.forEach(id=>subscribedRef.current.delete(id)))}setRevision(value=>value+1)}catch{}};
  ...
  useEffect(()=>{const timer=window.setInterval(()=>{if(document.hidden)return;const missing=[client.participants.joined,client.participants.active,client.participants.videoSubscribed].some(map=>(map.toArray()as unknown as ScreenPeer[]).some(peer=>peer.videoEnabled&&(!peer.videoTrack||peer.videoTrack.readyState!=="live")));if(missing)setRevision(value=>value+1)},8000);return()=>window.clearInterval(timer)},[client]);
  const cameras=useMemo(()=>{void revision;const items:Array<{id:string;name:string;track:MediaStreamTrack;local:boolean}>=[];const selfTrack=localTrack||client.self.videoTrack;if(selfTrack?.readyState==="live")items.push({id:"self",name:client.self.name||"You",track:selfTrack,local:true});const peers=new Map<string,ScreenPeer>();for(const peer of discovered)peers.set(peer.id,peer);for(const map of[client.participants.all,client.participants.joined,client.participants.active,client.participants.videoSubscribed])for(const peer of map.toArray()as unknown as ScreenPeer[])if(peer.id!==client.self.id)peers.set(peer.id,peer);for(const[id,fallback]of peers){const peer=(client.participants.videoSubscribed.get(id)||client.participants.active.get(id)||client.participants.joined.get(id)||client.participants.all.get(id)||fallback)as unknown as ScreenPeer|undefined;if(peer?.videoTrack?.readyState==="live")items.push({id:peer.id,name:peer.name||"Participant",track:peer.videoTrack,local:false})}return items},[client,discovered,localTrack,revision]);
...
export function ClassVideoContentShare({client,code,identity,sessionToken,mic,camera,manager,lang,onError,onMedia,apiBase="/api/classrooms"}:{client:RTKClient;code:string;identity:string;sessionToken:string;mic:boolean;camera:boolean;manager:boolean;lang:Lang;onError:(message:string)=>void;onMedia:(mic:boolean,camera:boolean)=>Promise<void>;apiBase?:string}){
  ...
  const publish=useCallback(async(track:MediaStreamTrack,next:"file"|"web"|"whiteboard")=>{try{cameraWasOn.current=camera;if(camera)await onMedia(mic,false);const approval=await fetch(`${apiBase}/${code}/media`,{method:"POST",headers:{"content-type":"application/json"},body:JSON.stringify({action:"media",identity,sessionToken,mic,camera:true})});if(!approval.ok)throw new Error("CONTENT_SHARE_NOT_ALLOWED");const previous=client.self.videoTrack;if(client.self.videoEnabled)await client.self.disableVideo();if(previous?.readyState==="live")previous.stop();if(trackRef.current?.readyState==="live")trackRef.current.stop();await client.self.enableVideo(track);trackRef.current=track;setSource(next);onError("")}catch{track.stop();onError(lang==="zh"?"无法开始内容共享。":"Could not start content sharing.");throw new Error("CONTENT_SHARE_FAILED")}},[apiBase,camera,client,code,identity,lang,mic,onError,onMedia,sessionToken]);
  const stop=useCallback(async()=>{if(client.self.videoEnabled)await client.self.disableVideo().catch(()=>undefined);if(trackRef.current?.readyState==="live")trackRef.current.stop();trackRef.current=undefined;setSource(undefined);await fetch(`${apiBase}/${code}/media`,{method:"POST",headers:{"content-type":"application/json"},body:JSON.stringify({action:"media",identity,sessionToken,mic,camera:false}),keepalive:true}).catch(()=>undefined);const restore=cameraWasOn.current;cameraWasOn.current=false;if(restore)await onMedia(mic,true)},[apiBase,client,code,identity,mic,onMedia,sessionToken]);
