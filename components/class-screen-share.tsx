...
export function ClassAudioScreenShare({ code, displayName, manager, lang, listening, onError, apiBase="/api/classes" }: { code:string; displayName:string; manager:boolean; lang:Lang; listening:boolean; onError:(message:string)=>void; apiBase?:string }) {
  const [client,setClient]=useState<RTKClient>();const[active,setActive]=useState(false);const[camera,setCamera]=useState(false);const[target,setTarget]=useState<Element|null>(null);const clientRef=useRef<RTKClient|undefined>(undefined);const joiningRef=useRef<Promise<RTKClient|undefined>|undefined>(undefined);const sharingRef=useRef(false);const cameraRef=useRef(false);
  ...
  const sharingChange=useCallback((sharing:boolean)=>{sharingRef.current=sharing;if(sharing&&cameraRef.current){const current=clientRef.current;cameraRef.current=false;setCamera(false);void current?.self.disableVideo().catch(()=>undefined)}void signal(sharing||cameraRef.current);if(!sharing&&!cameraRef.current)window.setTimeout(()=>void leave(),250)},[leave,signal]);
  const toggleCamera=useCallback(async()=>{const current=await connect();if(!current)return;try{if(cameraRef.current){await current.self.disableVideo();cameraRef.current=false;setCamera(false);if(!sharingRef.current){await signal(false);await leave()}}else{cameraRef.current=true;if(sharingRef.current){await current.self.disableScreenShare();sharingRef.current=false}current.self.setName(`${displayName} · Camera`);await current.self.enableVideo();setCamera(true);await signal(true)}onError("")}catch{cameraRef.current=false;setCamera(false);onError(lang==="zh"?"无法使用摄像头。请检查摄像头权限。":"Could not use the camera. Check camera permission.")}},[connect,displayName,lang,leave,onError,signal]);
  return <><ClassScreenShareButton client={client} manager={manager} lang={lang} onError={onError} ensurePublisher={connect} onSharingChange={sharingChange}/>{manager?<button className={camera?"on class-screen-share-button class-camera-share-button":"class-screen-share-button class-camera-share-button"} onClick={()=>void toggleCamera()} aria-label={lang==="zh"?(camera?"关闭共享摄像头":"共享摄像头"):(camera?"Stop camera share":"Share camera")}><svg viewBox="0 0 24 24" aria-hidden="true"><rect x="3" y="7" width="12" height="10" rx="2"/><path d="m15 10 5-3v10l-5-3z"/></svg></button>:null}{client&&active&&target?createPortal(<><ClassScreenShareStage client={client} lang={lang} listening={listening}/><ClassCompanionCameraStage client={client} lang={lang}/></>,target):null}</>;
}

function ClassCompanionCameraStage({client,lang}:{client:RTKClient;lang:Lang}){const[revision,setRevision]=useState(0);useEffect(()=>{const timer=window.setInterval(()=>setRevision(value=>value+1),500);return()=>window.clearInterval(timer)},[]);const cameras=useMemo(()=>{void revision;const items:Array<{id:string;name:string;track:MediaStreamTrack;local:boolean}>=[];if(client.self.videoEnabled&&client.self.videoTrack)items.push({id:"self",name:client.self.name||"You",track:client.self.videoTrack,local:true});const peers=new Map<string,ScreenPeer>();for(const map of[client.participants.joined,client.participants.active,client.participants.videoSubscribed])for(const peer of map.toArray()as unknown as ScreenPeer[])if(/ · (?:Screen|Camera)$/u.test(peer.name||""))peers.set(peer.id,peer);const ids=[...peers.keys()];if(ids.length)void client.participants.subscribe(ids,["video"]).catch(()=>undefined);for(const peer of peers.values())if(peer.videoEnabled&&peer.videoTrack)items.push({id:peer.id,name:peer.name||"Participant",track:peer.videoTrack,local:false});return items},[client,revision]);if(!cameras.length)return null;return <section className="class-screen-share-stage" aria-label={lang==="zh"?"共享摄像头":"Shared camera"}><header><strong>{lang==="zh"?"正在共享摄像头":"Camera sharing"}</strong><span>{cameras.length}</span></header>{cameras.map(item=><CompanionCameraPlayer key={item.id} item={item}/>)}</section>}
function CompanionCameraPlayer({item}:{item:{name:string;track:MediaStreamTrack;local:boolean}}){const ref=useRef<HTMLVideoElement>(null);useEffect(()=>{const video=ref.current;if(!video)return;video.srcObject=new MediaStream([item.track]);return()=>{video.srcObject=null}},[item.track]);return <div className="class-screen-share-player"><video ref={ref} autoPlay playsInline muted={item.local}/><span>{item.name.replace(/ · (?:Screen|Camera)$/u,"")} · Camera</span></div>}
