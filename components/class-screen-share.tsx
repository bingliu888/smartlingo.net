...
export function ClassScreenShareButton({ client, manager, lang, disabled, onError }: {
  client: RTKClient;
  manager: boolean;
  lang: Lang;
  disabled?: boolean;
  ...
  useEffect(() => {
    const update = ({ screenShareEnabled }: { screenShareEnabled: boolean }) => setSharing(screenShareEnabled);
  ...
  useEffect(() => {
    ...
    if (countdown === 0) {
      setCountdown(null);
      setBusy(true);
      void client.self.enableScreenShare()
        .then(() => { setSharing(true); onError(""); })
        .catch((error: unknown) => {
  ...
  }, [client, countdown, lang, onError]);
  if (!manager) return null;
  const label = sharing ? (lang === "zh" ? "停止共享" : "Stop sharing") : (lang === "zh" ? "共享屏幕" : "Share screen");
  return <>
    <button className={sharing ? "on class-screen-share-button" : "class-screen-share-button"} disabled={disabled || busy || countdown !== null} onClick={() => sharing ? void client.self.disableScreenShare().catch(() => onError(lang === "zh" ? "无法停止屏幕共享。" : "Could not stop screen sharing.")) : setCountdown(5)} aria-label={label} title={label}>
