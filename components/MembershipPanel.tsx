...
export function MembershipPanel({ lang }: { lang: InterfaceLanguage }) {
  ...
  const plan = maxActive ? (zh ? "旗舰版" : "Max") : (zh ? "免费版" : "Free");
  ...
  return <>
    ...
    <section className="member-panel">
      ...
      <div className="member-grid membership-tier-grid">
        ...
        <article className={maxActive ? "active" : ""}><small>{zh ? "旗舰版" : "MAX"}</small><strong>{zh ? "无广告 · 中高级课程" : "Ad-free · higher course levels"}</strong><p>{maxExpiry ? (zh ? `有效期至 ${maxExpiry}` : `Active through ${maxExpiry}`) : (zh ? "初级永久免费；中级和高级在一次性 7 天试用后需要旗舰版。6 个月 $119 或年度 $199，不自动续费。" : "Beginner is always free; Intermediate and Advanced require Max after one 7-day trial. Choose 6 months for $119 or annual access for $199; no auto-renewal.")}</p><Link href={`/${lang}/pricing`}>{zh ? "管理旗舰版" : "Manage Max"} →</Link></article>
