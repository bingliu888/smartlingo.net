...
export function RoleTutor({ lang, language, scene, level, role, speechLocale, initialMax, trialAvailable }: {
  ...
  return <section className="role-tutor-card" aria-label={zh ? "角色导师练习" : "Role tutor practice"}>
    ...
    <h2>{role}</h2>
    <p>{zh ? "与场景角色一对一练习。每轮最多 10 分钟、12 次回复；不会自动启动 Max 试用。" : "Practice one-to-one with a scene character. Each round lasts at most 10 minutes and 12 replies; it does not start a Max trial."}</p>
    ...
    {!max ? <div className="role-tutor-upgrade"><p>{trialAvailable ? (zh ? "需要有效 Max 方案。试用不会自动开启；首次使用可主动开启一次七天试用。" : "An active Max plan is required. The trial never starts automatically; you can explicitly start one seven-day trial.") : (zh ? "七天试用已结束。订阅 Max 后可以继续练习。" : "Your seven-day trial has ended. Subscribe to Max to continue.")}</p>{trialAvailable ? <button type="button" onClick={startTrial} disabled={busy}>{zh ? "开启七天 Max 试用" : "Start seven-day Max trial"}</button> : null} <Link href={`/${lang}/pricing`}>{zh ? "查看 Max" : "Explore Max"}</Link></div> : !sessionId ? <button type="button" onClick={start} disabled={busy}>{busy ? (zh ? "正在准备…" : "Preparing…") : (zh ? "开始一对一角色练习" : "Start 1:1 role-play")}</button> : <>
