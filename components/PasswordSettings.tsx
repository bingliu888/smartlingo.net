...
function readableError(code: string | undefined, t: Translator) {
  if (code === "CURRENT_PASSWORD_INCORRECT") return t("Your current password is incorrect.", "当前密码不正确。");
  if (code === "CURRENT_PASSWORD_REQUIRED") return t("Enter your current password.", "请输入当前密码。");
...
export function PasswordSettings({ lang }: { lang: InterfaceLanguage }) {
  ...
  const passwordEnabled = Boolean(user?.passwordEnabled || passwordSaved);
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      const response = await fetch("/api/account/password", {
        ...
        body: JSON.stringify({
          currentPassword: mode.requiresCurrentPassword ? currentPassword : undefined,
          password: newPassword,
        }),
      });
      const result = await response.json().catch(() => ({})) as { code?: string };
      if (!response.ok) { setError(readableError(result.code, t)); return; }
    ...
    } catch { setError(t("Could not save your password. Please try again.", "无法保存密码，请重试。")); } finally { setBusy(false); }
  }
  ...
  return <div className={styles.wrap}>
    ...
    <p>{mode.action === "update" ? t("Enter your current password.", "请输入当前密码。") : t("Add a password from this signed-in session. A recent email-code sign-in needs no extra code, and both sign-in methods will remain available.", "从当前已登录会话添加密码；近期使用邮箱验证码登录无需再次输入验证码，添加后两种登录方式都会保留。")}</p>
