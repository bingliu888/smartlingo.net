...
const sprintCopy = {
  en:{title:"Today’s Sprint",body:"Choose a language, time, and one of 21 progressive days",choose:"Choose language and day",close:"Close",heading:"Choose a language and today’s learning time",intro:"Ten minutes is selected by default. Every five minutes completes one vocabulary, reading, listening, writing, and speaking round.",stepLanguage:"1. Choose a language",stepTime:"2. Choose a time",stepDay:"3. Start from a day",minute:"min",round:"round",rounds:"rounds",start:"Start day {day} · {minutes}-minute Sprint",first:"Choose a language first"},
  zh:{title:"今日速成",body:"选择语言、时长和 21 天渐进学习日",choose:"选择语言与学习日",close:"关闭",heading:"选择语言和今天的学习时间",intro:"默认 10 分钟；每 5 分钟完成一轮词汇、阅读、听力、写作和口语。",stepLanguage:"1. 选择语言",stepTime:"2. 选择时长",stepDay:"3. 从第几天开始",minute:"分钟",round:"回合",rounds:"回合",start:"从第 {day} 天开始 · {minutes} 分钟",first:"请先选择语言"},
  ...
  return <>
    ...
    {open ? <div className="play-sprint-backdrop" role="presentation" onMouseDown={event => { if (event.target === event.currentTarget) setOpen(false); }}>
      <section className="play-sprint-picker" role="dialog" aria-modal="true" aria-labelledby="play-sprint-title">
        ...
        <h3>{"stepDay" in t ? t.stepDay : lang === "zh" ? "3. 从第几天开始" : "3. Start from a day"}</h3><LearningDayPicker lang={lang === "zh" ? "zh" : "en"} day={day} onChange={setDay}/>
        {language ? <Link className="play-sprint-start" href={`/${lang}/classes/course_${language}_basic/sprint?minutes=${minutes}&day=${day}&source=play&fresh=1`}>{t.start.replace("{minutes}", String(minutes)).replace("{day}", String(day))} →</Link> : <button className="play-sprint-start" type="button" disabled>{t.first}</button>}
      </section>
    </div> : null}
