  ...
  return <>
    ...
    {open ? <div className="play-sprint-backdrop" role="presentation" onMouseDown={event => { if (event.target === event.currentTarget) setOpen(false); }}>
      <section className="play-sprint-picker" role="dialog" aria-modal="true" aria-labelledby="play-sprint-title">
        ...
        {language ? <Link className="play-sprint-start" href={`/${lang}/classes/course_${language}_basic/sprint?minutes=${minutes}&day=${day}&source=play`}>{t.start.replace("{minutes}", String(minutes)).replace("{day}", String(day))} →</Link> : <button className="play-sprint-start" type="button" disabled>{t.first}</button>}
      </section>
    </div> : null}
