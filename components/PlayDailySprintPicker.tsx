...
export function PlayDailySprintPicker({ lang, initialLanguage }: { lang: "zh" | "en"; initialLanguage?: string }) {
  const zh = lang === "zh";
  ...
  return <>
    <button className="game-tile daily-sprint-tile" type="button" onClick={() => setOpen(true)}>
      <small>01 · DAILY SPRINT</small>
    ...
    {open ? <div className="play-sprint-backdrop" role="presentation" onMouseDown={event => { if (event.target === event.currentTarget) setOpen(false); }}>
      <section className="play-sprint-picker" role="dialog" aria-modal="true" aria-labelledby="play-sprint-title">
        ...
        {language ? <Link className="play-sprint-start" href={`/${lang}/classes/course_${language}_basic/sprint?minutes=${minutes}`}>{zh ? `开始 ${minutes} 分钟今日速成` : `Start today’s ${minutes}-minute Sprint`} →</Link> : <button className="play-sprint-start" type="button" disabled>{zh ? "请先选择语言" : "Choose a language first"}</button>}
      </section>
    </div> : null}
