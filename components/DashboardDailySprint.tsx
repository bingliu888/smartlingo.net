  ...
  return <section className={styles.sprint} aria-labelledby="dashboard-sprint-title">
    ...
    <div className={styles.rail}>{entries.map(({ language, course }) => {
      ...
      return <article key={language.code}>
        <div className={styles.language}><span>{language.code.toUpperCase()}</span><div><strong>{language.nativeName}</strong><small>{zh ? language.nameZh : language.nameEn}</small></div></div>
        <div className={styles.times} role="group" aria-label={zh ? `${language.nameZh}速成时长` : `${language.nameEn} Sprint time`}>{DURATIONS.map(value => <button type="button" aria-pressed={duration === value} className={duration === value ? styles.active : ""} onClick={() => setMinutes(current => ({ ...current, [language.code]: value }))} key={value}>{value}<small>{zh ? "分" : "m"}</small></button>)}</div>
        <p>{duration / 5} {zh ? "回合" : duration === 5 ? "round" : "rounds"} · {duration * 2} {zh ? "个词" : "words"} · {zh ? "五技能" : "five skills"}</p>
        <Link className={styles.start} href={`/${lang}/classes/${encodeURIComponent(course.id)}/sprint?minutes=${duration}`}>{zh ? `开始 ${duration} 分钟` : `Start ${duration} minutes`} →</Link>
      </article>;
