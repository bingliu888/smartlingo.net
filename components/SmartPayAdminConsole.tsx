  ...
  return <div className="smartpay-console-grid">
    ...
    <section className="smartpay-console-card smartpay-wide-card">
      ...
      {smartPay5PresetRows.length ? <div className="smartpay-rule-list smartpay-preset-list">{smartPay5PresetRows.map(({ preset, status }) => {
        ...
        const tier = preset.plan === "max_6m" ? "Max · 6 months"
        ...
        const scope = zh ? "平台产品" : "Platform product";
    ...
    <section className="smartpay-console-card smartpay-wide-card">
      ...
      <p>{zh ? "输入 PayerID 读取该账户最新 100 条交易；留空读取合约最新 25 条。付款钱包仅作为审计信息，PayerID 识别付款人，RefID 识别平台产品 Owner。" : "Enter a PayerID to read that account's latest 100 transactions, or leave it blank for the latest 25 contract transactions. The funding wallet is audit data only: PayerID identifies the payer and RefID identifies the platform product Owner."}</p>
