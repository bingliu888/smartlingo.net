...
export function SmartPayAdminConsole({
  ...
  return <div className="smartpay-console-grid">
    ...
    <section className="smartpay-console-card smartpay-wide-card">
      ...
      <p>{zh ? "课程的三个 3 个月价格产品与平台的 Max 6 个月、Max 年度及 1,000 个人工智能生成额度包分别保存为链上项目。课程付款以所选学习语言为 secondID；平台产品统一使用 platform。USDT 项目保存 100% USDT、100% 等值 GLC 与 10 亿 GLC 门槛，实际 USDT 比例保存在本站数据库。" : "Three 3-month course products plus Max 6-month, Max annual, and the 1,000-credit AIGC pack are stored as separate on-chain items. Course payments use the selected learning language as secondID; platform products use platform. USDT rules store full USDT/GLC equivalents and the 1-billion-GLC threshold, while the active mix stays in this site's database."}</p>
      {smartPay5PresetRows.length ? <div className="smartpay-rule-list smartpay-preset-list">{smartPay5PresetRows.map(({ preset, status }) => {
        ...
        const tier = preset.plan === "basic" ? (zh ? "初期课程" : "Beginner")
          : preset.plan === "intermediate" ? (zh ? "中级课程" : "Intermediate")
            : preset.plan === "advanced" ? (zh ? "高级课程" : "Advanced")
              : preset.plan === "max_6m" ? "Max · 6 months"
                : preset.plan === "max_12m" ? "Max · 12 months"
                  : (zh ? "人工智能生成 · 1,000 额度" : "AIGC · 1,000 credits");
