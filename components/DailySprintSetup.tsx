...
export function DailySprintSetup({ lang }: { lang: InterfaceLanguage }) {
  ...
  return <section className="sprint-setup" data-layout-fill="sprint-setup">
    ...
    {target ? <Link className="sprint-setup-start" href={`/${lang}/classes/course_${target.code}_basic/sprint?minutes=${minutes}&source=dashboard`}>{text("Start", "开始")} {minutes} {text("minutes", "分钟")} · {zh ? `用${sourceName}学${target.nameZh}` : `Learn ${target.nameEn} through ${sourceName}`} →</Link> : <button className="sprint-setup-start" type="button" disabled>{text("Choose a language first", "请先选择语言")}</button>}
  </section>;
}
