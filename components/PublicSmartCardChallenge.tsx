...
export function PublicSmartCardChallenge({ lang, token, gameMode = "practice" }: {
    ...
    const load = useCallback(async () => { const response = await fetch(`/api/smartcards/${encodeURIComponent(token)}`, { cache: "no-store" }); const payload = await response.json().catch(() => ({})) as Payload; if (!response.ok)
        throw new Error(payload.error); setData(payload); const resume=gameMode==="practice"?payload.practiceProgress:null; setIndex(resume?.currentIndex||0); setPoints(resume?.points||payload.policy?.startingPoints||100); evidence.current=(payload.deck?.cards||[]).map((item,index)=>resume?.cards[index]?{cardId:item.id,choices:[...resume.cards[index].choices],transcripts:[]}:{cardId:item.id,choices:[],transcripts:[]}); }, [gameMode,token]);
    ...
    return <main className="smart-game"><header><Link href={backHref}>← {gameMode === "challenge" ? (zh ? "挑战日历" : "Challenge calendar") : (zh ? "返回游戏" : "Back to games")}</Link><div className={`score ${scoreEffect ? (scoreEffect.amount > 0 ? "score-gain" : "score-loss") : ""}`}><span>{gameMode==="challenge"?(zh?"当前冠军成绩":"CURRENT WINNER"):(zh ? "课程积分" : "COURSE POINTS")}</span><strong>{gameMode==="challenge"?(currentLeader.score||"—"):points}</strong>{gameMode==="challenge"&&currentLeader.name?<small>{currentLeader.name}</small>:null}{gameMode!=="challenge"&&scoreEffect ? <i key={scoreEffect.id}>{scoreEffect.amount > 0 ? "+" : ""}{scoreEffect.amount}</i> : null}</div><p>{gameMode === "challenge" ? (zh ? "今日智慧卡挑战 · 每题一次机会" : "Today's challenge · one chance per question") : (data?.deck?.title || "SmartCard")}</p>{gameMode === "practice" ? <label className="smartcard-repeat-check"><input type="checkbox" checked={repeatAfterMe} onChange={event => setRepeatAfterMe(event.target.checked)}/><span>{zh ? "三次跟读评分（默认关闭）" : "Three scored repeats (off by default)"}</span></label> : null}</header>
