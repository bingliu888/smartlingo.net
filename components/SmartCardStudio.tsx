...
type Card = { id: string; form: string; pronunciation: string; meaningEn: string; meaningZh: string; sceneKey: string; difficulty: number; frequencyDegree: number; gradeLevel: number };
...
export function SmartCardStudio({ lang, classId }: { lang: "en" | "zh"; classId: string }) {
  ...
  return <section className="sl-smartcard-studio" aria-labelledby="sl-smartcard-title">
    ...
    <p>{zh ? "词卡只取自已发布的审核词库。挑战首次达到 80 分可得 10 点，每日最多 50 点；奖励积分用于站内数字奖励，不是现金、课程费抵扣或可交易资产。自己挑战自己的词卡不发奖励积分。" : "Cards use published, reviewed vocabulary only. A first score of 80 earns 10 points, capped at 50 daily. Reward points are for in-product digital rewards—not cash, course-fee credit, or a tradable asset. Self-challenges do not earn reward points."}</p>
    ...
    {deck ? <div className="sl-smartcard-deck"><div className="sl-smartcard-share"><strong>{deck.title}</strong><span>{deck.bestScore === null ? (zh ? "尚未挑战" : "Not challenged") : `${zh ? "最佳" : "Best"} ${deck.bestScore}/100`}</span><button type="button" onClick={() => void share(deck)}>{zh ? "复制分享链接" : "Copy share link"}</button></div>
      <div className="sl-smartcard-grid">{deck.cards.map((card, index) => <article key={card.id}>
        <small>{String(index + 1).padStart(2, "0")} · {card.sceneKey}</small><strong>{revealed[card.id] ? card.form : (zh ? card.meaningZh : card.meaningEn)}</strong>{revealed[card.id] && card.pronunciation ? <span>{card.pronunciation}</span> : null}<em>{zh ? "难度" : "Difficulty"} {card.difficulty}/5 · {zh ? "常用度" : "Frequency"} {card.frequencyDegree}/10 · {vocabularyGradeLabel(card.gradeLevel, zh ? "zh" : "en")}</em>
    ...
    <style>{`.sl-smartcard-grid article>em{color:#087d62;font-size:12px;font-style:normal;font-weight:850}`}</style>
