  ...
  if (detail) {
    ...
    return <section className="smartlingo-class-studio">
      ...
      <div className="class-detail-grid">
        ...
        {joined && <article className="class-placement-card"><span>5 SKILLS</span><h2>{t.fiveSkillsTitle}</h2><p>{t.fiveSkillsBody}</p><ul><li>✓ {lang === "zh" ? "词汇" : "Vocabulary"}</li><li>✓ {lang === "zh" ? "口语" : "Speaking"}</li><li>✓ {lang === "zh" ? "听力" : "Listening"}</li><li>✓ {lang === "zh" ? "写作" : "Writing"}</li><li>✓ {lang === "zh" ? "测验" : "Quiz"}</li></ul><div className="class-learning-actions"><Link className="primary-button" href={`/${lang}/classes/${item.id}/learn`}>{t.dailyLearning} →</Link><Link className="secondary-button" href={`/${lang}/learning-log`}>{t.calendar} →</Link></div></article>}
