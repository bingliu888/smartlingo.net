...
export function CourseClassroomTile({ classId, lang, compact = false }: { classId: string; lang: "en" | "zh"; compact?: boolean }) {
  ...
  const rooms = [
    { key: "teaching", icon: "▣", eyebrow: zh ? "WEBINAR · 音视频" : "WEBINAR · A/V", title: zh ? "课程教室" : "Teaching room", copy: state?.teachingRoom.title || (zh ? "管理员授课与协办主持演讲" : "Administrator teaching and co-host speakers"), live: state?.teachingRoom.live, code: state?.teachingRoom.code },
  ...
  return <section className={`course-room-grid${compact ? " compact" : ""}`} data-layout-fill={compact ? undefined : "course-classrooms"} aria-label={zh ? "课程课程教室与练习室" : "Course teaching and practice rooms"}>
    ...
    <style>{`.class-detail-grid .course-course room-tile{grid-column:auto!important}`}</style>
