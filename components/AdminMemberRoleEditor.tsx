...
export function AdminMemberRoleEditor({ memberId, initialRole, lang, locked }: { memberId: string; initialRole: "member" | "admin"; lang: "en" | "zh"; locked: boolean }) {
  ...
  return <div className="admin-role-editor"><label htmlFor="member-role">{zh ? "平台角色" : "Platform role"}</label><select id="member-role" value={role} disabled={locked || busy} onChange={(event) => setRole(event.target.value as "member" | "admin")}><option value="member">{zh ? "会员" : "Member"}</option><option value="admin">{zh ? "管理员" : "Admin"}</option></select><button type="button" onClick={save} disabled={locked || busy || role === initialRole}>{busy ? (zh ? "保存中…" : "Saving…") : (zh ? "保存角色" : "Save role")}</button>{locked && <p>{zh ? "默认管理员角色受保护，不能降级。" : "The bootstrap administrator role is protected."}</p>}{status && <p role="status">{status}</p>}</div>;
}
