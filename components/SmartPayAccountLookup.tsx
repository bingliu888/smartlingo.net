...
export function SmartPayAccountLookup({lang}:{lang:InterfaceLanguage}){
  ...
  useEffect(()=>{void fetch("/api/billing/status",{cache:"no-store"}).then(r=>r.json()).then(data=>{const rows=(data.cryptoSettings||[]).filter((item:CryptoPaymentSetting)=>item.smartPay4Contract);setSettings(rows);setSettingId(rows[0]?.id||"");}).catch(()=>setMessage(t("Payment networks are temporarily unavailable.","付款网络暂时不可用。")));},[t]);
