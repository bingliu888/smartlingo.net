...
export function CoursePaymentActions({ lang, classId, targetLanguage, packageTier, initialMonths, departmentId }: {
  lang: SiteLanguage; classId: string; targetLanguage: string; packageTier: SmartLingoPackageTier;
  initialMonths: SmartLingoCourseDurationMonths; departmentId?: string;
}) {
  ...
  async function payCard() {
    ...
    const response = await fetch("/api/billing/card/checkout", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ classId, targetLanguage, months, lang, departmentId }) });
  ...
  return <div className="course-payment-actions">
    ...
    {months===3&&!departmentId?<Link className="crypto-payment-button" href={`/${lang}/classes/${encodeURIComponent(classId)}/pay/crypto?language=${encodeURIComponent(targetLanguage)}&months=3`}>{t("Pay 3 months with Polygon USDT or GLC","使用 Polygon USDT 或 GLC 支付三个月")}</Link>:null}
