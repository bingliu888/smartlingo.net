...
export function CoursePaymentActions({ lang, classId, priceCents, firstMonthFree }: { lang: "en" | "zh"; classId: string; priceCents: number; firstMonthFree: boolean }) {
  ...
  useEffect(() => { let active = true; void fetch(`/api/billing/credits/redeem?classId=${encodeURIComponent(classId)}`, { cache: "no-store" }).then(async response => ({ ok: response.ok, data: await response.json().catch(() => ({})) })).then(result => { if (active && result.ok) setCredit(result.data); }).catch(() => undefined); return () => { active = false; }; }, [classId]);
  ...
  return <div className="course-payment-actions">
    ...
    {firstMonthFree ? <button className="free-month-button" type="button" onClick={() => void startFreeMonth()} disabled={busy}>{busy ? (zh ? "正在开通…" : "Starting…") : (zh ? "开始免费首月" : "Start free first month")}</button> : null}
