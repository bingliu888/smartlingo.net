...
export function DailySprint({ lang, classId, durationMinutes, publicPlay = false }: { lang: "zh" | "en"; classId: string; durationMinutes: 5 | 10 | 15 | 20; publicPlay?: boolean }) {
  ...
  useEffect(() => {
    ...
    fetch(`/api/classes/${encodeURIComponent(classId)}/sprint`, { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ action: "start", durationMinutes, lang, timeZone, source: publicPlay ? "play" : undefined }) })
      ...
      .then(data => { const initial = Array.from({ length: data.plan.rounds.length }, () => ({} as SprintAnswer)); setRunId(data.runId); setPlan(data.plan); setCourseTitle(data.courseTitle); setAnonymous(data.anonymous === true); setResponses(initial); responsesRef.current = initial; setRemainingSeconds(durationMinutes * 60); })
  ...
  function speak(text: string, rate = .78, onEnd?: () => void) {
    ...
    window.speechSynthesis.cancel(); const utterance = new SpeechSynthesisUtterance(text); let settled = false;
    const finish = () => { if (settled) return; settled = true; window.clearTimeout(speechTimerRef.current); onEnd?.(); };
    utterance.lang = LOCALES[plan?.language || "en"] || "en-US"; utterance.rate = rate; utterance.onend = finish; utterance.onerror = finish;
  ...
  function beginSpeechSeries(expected: string, pronunciation: string, kind: SpeechKind, onComplete: () => void) {
    resetSpeech(kind); setSpeechBusy(true); let bestTranscript = "", bestScore = -1;
    const finishSeries = (scores: number[]) => {
      const average = scores.length ? Math.round(scores.reduce((sum, score) => sum + score, 0) / scores.length) : 0;
      if (kind === "dialogue") update({ dialogueTranscript: bestTranscript });
      setSpeechBusy(false); setSpeechAttempted(true); setSpeechFeedback(zh ? `三次跟读完成，平均 ${average} 分，自动进入下一项。` : `Three repeats complete. Average ${average}. Moving on automatically.`);
      transitionTimerRef.current = window.setTimeout(onComplete, 1300);
    };
  ...
  function chooseVocabulary(choice: string) {
    ...
    setVocabChoice(choice); setVocabChecked(true); const current = responsesRef.current[roundIndex] || {};
  ...
  function chooseReading(choice: string) {
    ...
    setReadingChoice(choice); setReadingChecked(true); update({ reading: choice });
    transitionTimerRef.current = window.setTimeout(() => setStage("listening"), 1400);
  }
  function startDialogue() {
    if (!round) return;
    setStage("dialogue"); resetSpeech("dialogue");
    transitionTimerRef.current = window.setTimeout(() => repeatAfterMe ? beginSpeechSeries(round.dialogue.audioText, "", "dialogue", nextRound) : speak(round.dialogue.audioText, .72, () => { update({ dialogueTranscript: "" }); transitionTimerRef.current = window.setTimeout(nextRound, 700); }), 100);
  }
  ...
  return <section className="daily-sprint">
    <header className="sprint-head"><div><p>TODAY’S SPRINT · {durationMinutes} MIN</p><h1>{zh ? "今日速成" : "Today’s Sprint"}</h1><span>{courseTitle} · {roundIndex + 1}/{plan.rounds.length} {zh ? "回合" : "rounds"}</span></div><aside><div className="sprint-countdown" aria-live="polite"><small>{zh ? "剩余时间" : "TIME LEFT"}</small><b>{String(Math.floor(remainingSeconds / 60)).padStart(2, "0")}:{String(remainingSeconds % 60).padStart(2, "0")}</b></div><button aria-pressed={repeatAfterMe} onClick={() => setRepeatAfterMe(!repeatAfterMe)}>{zh ? `跟我读：${repeatAfterMe ? "开" : "关"}` : `Repeat: ${repeatAfterMe ? "On" : "Off"}`}</button><strong>{progress}%</strong></aside></header><div className="sprint-progress"><span style={{ width: `${progress}%` }}/></div>
    {stage === "vocabulary" && word ? <article className="sprint-card sprint-vocabulary"><small>01 · {labels.vocabulary} · {wordIndex + 1}/{round.vocabulary.length}</small><h3>{zh ? "选择正确的意思，或点击卡片翻面" : "Choose the meaning, or flip the card"}</h3><button type="button" className={`sprint-flip-card ${vocabFlipped ? "flipped" : ""}`} onClick={() => setVocabFlipped(value => !value)}><span className="front"><strong dir={plan.language === "ar" ? "rtl" : "ltr"}>{word.form}</strong>{word.pronunciation ? <b>{word.pronunciation}</b> : null}<em>{zh ? "点击查看意思" : "Tap to see the meaning"}</em></span><span className="back"><strong>{word.meaning}</strong><b>{word.pronunciation}</b><em>{zh ? "再点一次返回" : "Tap again to return"}</em></span></button><div className="sprint-options vocab-options">{choices.map(option => <button type="button" className={vocabChoice === option.id ? "selected" : ""} disabled={vocabChecked} onClick={() => chooseVocabulary(option.id)} key={option.id}>{option.meaning}</button>)}</div>{vocabChecked ? <aside className={vocabularyCorrect ? "correct" : "incorrect"}><strong>{vocabularyCorrect ? (zh ? "✓ 太棒了！" : "✓ Great!") : (zh ? "× 答案不对" : "× Not quite")}</strong>{!vocabularyCorrect ? <p><b>{zh ? "正确答案：" : "Correct answer: "}</b>{word.meaning}</p> : null}<p>{repeatAfterMe ? speechFeedback || (zh ? "准备自动跟读三遍…" : "Preparing three automatic repeats…") : (zh ? "跟读已关闭，示范后自动进入下一词。" : "Repeat is off. Moving on after the model audio.")}</p>{repeatAfterMe ? <div className="sprint-speech-analysis" aria-label={zh ? "三次跟读分析" : "Three pronunciation analyses"}><span>{[1,2,3].map(turn => <b className={turn <= speechScores.length ? "scored" : turn === speechRound ? "active" : ""} key={turn}>{speechScores[turn - 1] ?? turn}</b>)}</span>{speechScores.length ? <strong>{zh ? "当前平均" : "Current average"} {Math.round(speechScores.reduce((sum, score) => sum + score, 0) / speechScores.length)}</strong> : null}</div> : null}{speechTranscript ? <p><b>{zh ? "设备听到：" : "Device heard: "}</b>{speechTranscript}</p> : null}</aside> : null}</article> : null}
    {stage === "reading" ? <article className="sprint-card sprint-reading"><small>02 · {labels.reading}</small><h3>{zh ? "自动跟读句子三遍，再选择正确意思" : "Repeat the sentence three times, then choose its meaning"}</h3><blockquote dir={plan.language === "ar" ? "rtl" : "ltr"}>{round.reading.prompt}</blockquote>{speechKind === "reading" ? <aside className={speechTranscript ? "correct" : "incorrect"}><p>{speechFeedback || (zh ? "正在准备句子跟读…" : "Preparing sentence practice…")}</p>{repeatAfterMe ? <div className="sprint-speech-analysis"><span>{[1,2,3].map(turn => <b className={turn <= speechScores.length ? "scored" : turn === speechRound ? "active" : ""} key={turn}>{speechScores[turn - 1] ?? turn}</b>)}</span></div> : null}{speechTranscript ? <p><b>{zh ? "设备听到：" : "Device heard: "}</b>{speechTranscript}</p> : null}</aside> : null}<div className="sprint-options">{round.reading.options.map(option => <button className={readingChoice === option.id ? "selected" : ""} disabled={readingChecked || !speechAttempted} onClick={() => chooseReading(option.id)} key={option.id}>{option.label}</button>)}</div>{readingChecked ? <aside className={readingChoice === round.reading.answerId ? "correct" : "incorrect"}><strong>{readingChoice === round.reading.answerId ? (zh ? "✓ 回答正确" : "✓ Correct") : (zh ? "× 答案不对" : "× Not quite")}</strong>{readingChoice !== round.reading.answerId ? <p><b>{zh ? "正确答案：" : "Correct answer: "}</b>{readingCorrectLabel}</p> : null}<p>{zh ? "即将自动进入听力组句…" : "Moving to listening automatically…"}</p></aside> : null}</article> : null}
    {stage === "listening" ? <article className="sprint-card"><small>03 · {labels.listening}</small><SentenceBuilderRound autoAdvance lang={lang} mode="listening" speechLocale={LOCALES[plan.language]} exercises={[round.listening]} onComplete={answer => { const parsed = JSON.parse(answer) as string[]; update({ listening: parsed[0] || "" }); setStage("writing"); }}/></article> : null}
    {stage === "writing" ? <article className="sprint-card"><small>04 · {labels.writing}</small><SentenceBuilderRound autoAdvance lang={lang} mode="writing" speechLocale={LOCALES[plan.language]} exercises={[round.writing]} onComplete={answer => { const parsed = JSON.parse(answer) as string[]; update({ writing: parsed[0] || "" }); startDialogue(); }}/></article> : null}
    {stage === "dialogue" ? <article className="sprint-card sprint-dialogue"><small>05 · {labels.dialogue}</small><h3>{zh ? "最后自动跟 AI 说一句" : "Finish with one automatic AI speaking turn"}</h3><blockquote dir={plan.language === "ar" ? "rtl" : "ltr"}>{round.dialogue.audioText}</blockquote><p>{round.dialogue.prompt}</p><aside className={speechTranscript ? "correct" : "incorrect"}><p>{speechFeedback || (zh ? "正在准备口语分析…" : "Preparing speech analysis…")}</p>{repeatAfterMe ? <div className="sprint-speech-analysis"><span>{[1,2,3].map(turn => <b className={turn <= speechScores.length ? "scored" : turn === speechRound ? "active" : ""} key={turn}>{speechScores[turn - 1] ?? turn}</b>)}</span></div> : null}{speechTranscript ? <p><b>{zh ? "设备听到：" : "Device heard: "}</b>{speechTranscript}</p> : null}</aside></article> : null}
