...
export function DailySprint({ lang, classId, durationMinutes, publicPlay = false }: { lang: "zh" | "en"; classId: string; durationMinutes: 5 | 10 | 15 | 20; publicPlay?: boolean }) {
  ...
  function nextWord() {
    ...
    transitionTimerRef.current = window.setTimeout(() => beginSpeechSeries(round.reading.prompt, "", "reading", () => setSpeechAttempted(true)), 100);
  }
  ...
  function startDialogue() {
    ...
    transitionTimerRef.current = window.setTimeout(() => beginSpeechSeries(round.dialogue.audioText, "", "dialogue", nextRound), 100);
  }
  ...
  return <section className="daily-sprint">
    ...
    {stage === "vocabulary" && word ? <article className="sprint-card sprint-vocabulary"><small>{vocabularyWasMissed ? (zh ? "以前错过" : "PREVIOUS MISTAKE") : (zh ? "新词" : "NEW WORD")} · {labels.vocabulary} · {wordIndex + 1}/{round.vocabulary.length}</small><div className="sprint-word-metrics"><span>{zh ? "难度" : "Difficulty"} {word.difficulty}/5</span><span>{zh ? "常用度" : "Frequency"} {word.frequencyDegree}/10</span></div><h3>{zh ? "选择正确的意思，或点击卡片翻面" : "Choose the meaning, or flip the card"}</h3><button type="button" className={`sprint-flip-card ${vocabFlipped ? "flipped" : ""}`} onClick={() => setVocabFlipped(value => !value)}><span className="front"><strong dir={plan.language === "ar" ? "rtl" : "ltr"}>{word.form}</strong>{word.pronunciation ? <b>{word.pronunciation}</b> : null}<em>{zh ? "点击查看意思" : "Tap to see the meaning"}</em></span><span className="back"><strong>{word.meaning}</strong><b>{word.pronunciation}</b><em>{zh ? "再点一次返回" : "Tap again to return"}</em></span></button><div className="sprint-options vocab-options">{choices.map(option => <button type="button" aria-pressed={vocabChoice === option.id} className={vocabChoice === option.id ? "selected" : ""} disabled={vocabChecked} onClick={() => chooseVocabulary(option.id)} key={option.id}>{option.meaning}</button>)}</div>{!vocabChecked ? <footer><button type="button" className="primary-button" disabled={!vocabChoice} onClick={checkVocabulary}>{zh ? "检查" : "Check"}</button></footer> : <aside className={vocabularyCorrect ? "correct" : "incorrect"}><strong>{vocabularyCorrect ? (zh ? "✓ 太棒了！" : "✓ Great!") : (zh ? "× 答案不对" : "× Not quite")}</strong>{!vocabularyCorrect ? <p><b>{zh ? "正确答案：" : "Correct answer: "}</b>{word.meaning}</p> : null}<nav className="sprint-speed"><button type="button" onClick={() => speak(word.form,.86)}>🔊 {zh ? "正常语速" : "Normal"}</button><button type="button" onClick={() => speak(word.form,.58)}>🐢 {zh ? "慢速" : "Slow"}</button></nav><p>{speechKind === "vocabulary" ? speechFeedback : (zh ? "继续后跟读三次，每次都会评分。" : "Continue for three scored follow-me attempts.")}</p>{speechKind === "vocabulary" ? <div className="sprint-speech-analysis" aria-label={zh ? "三次跟读分析" : "Three pronunciation analyses"}><span>{[1,2,3].map(turn => <b className={turn <= speechScores.length ? "scored" : turn === speechRound ? "active" : ""} key={turn}>{speechScores[turn - 1] ?? turn}</b>)}</span>{speechScores.length ? <strong>{zh ? "当前平均" : "Current average"} {Math.round(speechScores.reduce((sum, score) => sum + score, 0) / speechScores.length)}</strong> : null}</div> : null}{speechTranscript ? <p><b>{zh ? "设备听到：" : "Device heard: "}</b>{speechTranscript}</p> : null}{!speechBusy ? <footer><button type="button" className="primary-button" onClick={() => speechKind === "vocabulary" && speechAttempted ? speechContinueRef.current?.() : startVocabularySpeaking()}>{zh ? "继续" : "Continue"}</button></footer> : null}</aside>}</article> : null}
    {stage === "reading" ? <article className="sprint-card sprint-reading"><small>02 · {labels.reading}</small><h3>{zh ? "跟读句子三遍，再选择正确意思" : "Repeat the sentence three times, then choose its meaning"}</h3><blockquote dir={plan.language === "ar" ? "rtl" : "ltr"}>{round.reading.prompt}</blockquote><nav className="sprint-speed"><button type="button" onClick={() => speak(round.reading.prompt,.86)}>🔊 {zh ? "正常语速" : "Normal"}</button><button type="button" onClick={() => speak(round.reading.prompt,.58)}>🐢 {zh ? "慢速" : "Slow"}</button></nav>{speechKind === "reading" ? <aside className={speechTranscript ? "correct" : "incorrect"}><p>{speechFeedback || (zh ? "正在准备句子跟读…" : "Preparing sentence practice…")}</p><div className="sprint-speech-analysis"><span>{[1,2,3].map(turn => <b className={turn <= speechScores.length ? "scored" : turn === speechRound ? "active" : ""} key={turn}>{speechScores[turn - 1] ?? turn}</b>)}</span></div>{speechTranscript ? <p><b>{zh ? "设备听到：" : "Device heard: "}</b>{speechTranscript}</p> : null}</aside> : null}<div className="sprint-options">{round.reading.options.map(option => <button type="button" aria-pressed={readingChoice === option.id} className={readingChoice === option.id ? "selected" : ""} disabled={readingChecked || !speechAttempted} onClick={() => chooseReading(option.id)} key={option.id}>{option.label}</button>)}</div>{!readingChecked ? <footer><button type="button" className="primary-button" disabled={!readingChoice || !speechAttempted} onClick={checkReading}>{zh ? "检查" : "Check"}</button></footer> : <aside className={readingChoice === round.reading.answerId ? "correct" : "incorrect"}><strong>{readingChoice === round.reading.answerId ? (zh ? "✓ 回答正确" : "✓ Correct") : (zh ? "× 答案不对" : "× Not quite")}</strong>{readingChoice !== round.reading.answerId ? <p><b>{zh ? "正确答案：" : "Correct answer: "}</b>{readingCorrectLabel}</p> : null}<footer><button type="button" className="primary-button" onClick={() => setStage("listening")}>{zh ? "继续" : "Continue"}</button></footer></aside>}</article> : null}
    ...
    {stage === "writing" ? <article className="sprint-card"><small>04 · {labels.writing}</small><SentenceBuilderRound lang={lang} mode="writing" speechLocale={LOCALES[plan.language]} exercises={[round.writing]} onComplete={answer => { const parsed = JSON.parse(answer) as string[]; update({ writing: parsed[0] || "" }); startDialogue(); }}/></article> : null}
    {stage === "dialogue" ? <article className="sprint-card sprint-dialogue"><small>05 · {labels.dialogue}</small><h3>{zh ? "最后跟 AI 说一句，完成三次评分跟读" : "Finish with three scored AI speaking attempts"}</h3><blockquote dir={plan.language === "ar" ? "rtl" : "ltr"}>{round.dialogue.audioText}</blockquote><p>{round.dialogue.prompt}</p><nav className="sprint-speed"><button type="button" onClick={() => speak(round.dialogue.audioText,.86)}>🔊 {zh ? "正常语速" : "Normal"}</button><button type="button" onClick={() => speak(round.dialogue.audioText,.58)}>🐢 {zh ? "慢速" : "Slow"}</button></nav><aside className={speechTranscript ? "correct" : "incorrect"}><p>{speechFeedback || (zh ? "正在准备口语分析…" : "Preparing speech analysis…")}</p><div className="sprint-speech-analysis"><span>{[1,2,3].map(turn => <b className={turn <= speechScores.length ? "scored" : turn === speechRound ? "active" : ""} key={turn}>{speechScores[turn - 1] ?? turn}</b>)}</span></div>{speechTranscript ? <p><b>{zh ? "设备听到：" : "Device heard: "}</b>{speechTranscript}</p> : null}{speechAttempted && !speechBusy ? <footer><button type="button" className="primary-button" onClick={() => speechContinueRef.current?.()}>{zh ? "继续" : "Continue"}</button></footer> : null}</aside></article> : null}
