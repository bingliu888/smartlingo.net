...
export function PlatformPlans({ lang, checkout, sessionId, compact = false }: { lang: InterfaceLanguage; checkout?: string; sessionId?: string; compact?: boolean }) {
  ...
  return <section className={`platform-pricing${compact ? " platform-pricing-compact" : ""}`}>
    <header className="platform-pricing-heading"><p className="section-kicker">{t("SMARTLINGO MEMBERSHIP", "SMARTLINGO 会员方案")}</p><h1>{t("Learn Beginner free. Go further with Max.", "初级永久免费，用 Max 更进一步。")}</h1><p>{t("Free includes every Beginner course with ads and real-life missions with an uncoached reply check. Max unlocks higher course levels and removes ads. Your one-time 7-day trial starts only when you explicitly enroll in a higher-level course; Max never renews automatically.", "免费方案包含全部带广告的初级课程，以及带无提示应答检验的生活任务。Max 解锁更高课程等级并去除广告。一次性 7 天试用仅在您明确报名更高等级课程时启动；Max 不会自动续费。")}</p></header>
