...
export function PlatformPlans({ lang, checkout, sessionId, compact = false }: { lang: InterfaceLanguage; checkout?: string; sessionId?: string; compact?: boolean }) {
  ...
  return <section className={`platform-pricing${compact ? " platform-pricing-compact" : ""}`}>
    <header className="platform-pricing-heading"><p className="section-kicker">{t("SMARTLINGO MEMBERSHIP", "SMARTLINGO 会员方案")}</p><h1>{t("Learn Beginner free. Go further with Max.", "初级永久免费，用旗舰版更进一步。")}</h1><p>{t("Free includes every Beginner course with ads and real-life missions with an uncoached reply check. Max unlocks higher course levels, ad-free learning, and one-to-one AI role-play. Your one-time 7-day trial starts only when you explicitly enroll in a higher-level course or choose to start it for the role tutor; Max never renews automatically.", "免费方案包含全部带广告的初级课程，以及带无提示应答检验的生活任务。旗舰版解锁更高课程等级、无广告学习和一对一 AI 角色练习。一次性 7 天试用仅在您明确报名更高等级课程或主动为角色导师开启时启动；旗舰版不会自动续费。")}</p></header>
    ...
    <div className="platform-plan-grid">
      <article className="platform-free-card">
        ...
        <h2>{t("Free", "免费")}</h2>
        <strong>$0</strong>
        <p>{t("Learn every Beginner course with ads, including daily practice, SmartCard, speaking scenarios, Community, and the public AI study partner. Visiting Beginner never starts Max.", "免费学习全部带广告的初级课程，包括今日练习、SmartCard、生活口语、社区和公开人工智能学伴；进入初级课程不会启动旗舰版。")}</p>
      ...
      {maxProducts.map(product => <article className="platform-max-card" key={product.id}>
        <small>{t("ONE COMPLETE MAX PLAN", "完整的旗舰版方案")}</small>
    ...
    <article className="aigc-credit-card" id="aigc-credits"><div><small>{t("AIGC TOKEN CREDIT", "人工智能生成额度")}</small><h2>{t("Create image, audio, and video media", "生成图片、音频与视频媒体")}</h2><p>{t("One prepaid pack adds 1,000 non-cash credits. The balance is independent from Max and has no exchange or withdrawal value.", "一次购买增加 1,000 个非现金额度；余额与旗舰版相互独立，不可兑换或提现。")}</p></div><div className="aigc-credit-price"><b>{state.aigcCredits ?? 0}</b><span>{t("CREDIT", "额度")}</span><small>{credit.displayPrice} / 1,000</small><small>{t(`Image ${SMARTLINGO_AIGC_CREDIT_COSTS.image} · Audio ${SMARTLINGO_AIGC_CREDIT_COSTS.audio} · Video ${SMARTLINGO_AIGC_CREDIT_COSTS.video}`, `图片 ${SMARTLINGO_AIGC_CREDIT_COSTS.image} · 音频 ${SMARTLINGO_AIGC_CREDIT_COSTS.audio} · 视频 ${SMARTLINGO_AIGC_CREDIT_COSTS.video}`)}</small>{paymentActions(credit.id)}</div></article>
  </section>;
}
