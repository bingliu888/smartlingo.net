...
export function CollegeCoordinatorComplete({ lang, sessionId }: { lang: InterfaceLanguage; sessionId: string }) {
  ...
  return <section className="college-complete"><p>COLLEGE SUPERVISOR</p><h1>{state === "loading" ? (zh ? "正在确认购买…" : "Confirming purchase…") : state === "done" ? (zh ? "学院总监资格已启用" : "College Supervisor is active") : (zh ? "暂时无法确认购买" : "Purchase could not be confirmed")}</h1><p>{state === "done" ? (zh ? "现在可以创建招生学院，并在方案额度内建立语言部门。" : "You can now create an enrollment college and language departments within your package quota.") : state === "error" ? (zh ? "请返回“我的学院”重试；不会重复收费。" : "Return to My colleges and retry. You will not be charged twice.") : (zh ? "请不要关闭页面。" : "Please keep this page open.")}</p><Link className="primary-button" href={`/${lang}/colleges/mine`}>{zh ? "返回我的学院" : "Back to My colleges"} →</Link></section>;
}
