...
import { addCourseSubscriptionMonths, courseSubscriptionPackage, type SmartLingoPackageTier } from "@/lib/smartlingo-course-packages";

type Course = { id: string; title: string; packageTier: SmartLingoPackageTier };
...
async function quote(request: Request) {
  ...
  const course = await database.prepare(`SELECT id,title,package_tier AS packageTier
  ...
  const selectedPackage=courseSubscriptionPackage(course.packageTier,3);
  if(!selectedPackage)return {error:Response.json({error:"Course package not found"},{status:404})} as const;
  ...
  return { user, database, course, selectedPackage, balancePoints } as const;
}

export async function GET(request: Request) {
  const value = await quote(request); if ("error" in value) return value.error;
  return Response.json({ balancePoints: value.balancePoints, requiredPoints: value.selectedPackage.priceCents, eligibleForPackage: value.balancePoints >= value.selectedPackage.priceCents, package: value.selectedPackage, pointsPerUsd: 100 });
}

export async function POST(request: Request) {
  const value = await quote(request); if ("error" in value) return value.error;
  if (value.balancePoints < value.selectedPackage.priceCents) return Response.json({ error: "Not enough course points for the three-month package", balancePoints: value.balancePoints, requiredPoints: value.selectedPackage.priceCents }, { status: 409 });
  const now = Math.floor(Date.now() / 1000); const monthKey = new Date(now * 1000).toISOString().slice(0, 7);
  const sourceId = `course-package-3m:${value.course.id}:${monthKey}`;
  ...
  if (existing) return Response.json({ error: "Course points were already used for this three-month package" }, { status: 409 });
  ...
  const periodStart = Math.max(now, Number(current?.currentPeriodEndsAt || 0)); const periodEnds = addCourseSubscriptionMonths(periodStart,3);
  ...
  try {
    await value.database.batch([
      value.database.prepare(`INSERT INTO smartlingo_course_credit_redemptions
        ...
        VALUES(?,?,?,?,?,?,'credit_only',?,'applied',?,?)`).bind(redemptionId, value.user.id, value.course.id, value.selectedPackage.priceCents, value.selectedPackage.priceCents, value.selectedPackage.priceCents, sourceId, now, now),
      value.database.prepare(`INSERT INTO smartlingo_course_credit_ledger
        ...
        VALUES(?,?,?,'course_redeem','course_month',?,?,?,?)`).bind(ledgerId, value.user.id, -value.selectedPackage.priceCents, sourceId, localDate, `Three-month package for ${value.course.title}`, now),
      value.database.prepare(`INSERT INTO smartlingo_course_subscriptions
        ...
        .bind(createId(), value.course.id, value.user.id, value.selectedPackage.priceCents, now, now, periodEnds, `credit:${redemptionId}`, now, now),
  ...
  return Response.json({ redeemed: true, pointsUsed: value.selectedPackage.priceCents, periodEnds, classId: value.course.id,package:value.selectedPackage });
}
