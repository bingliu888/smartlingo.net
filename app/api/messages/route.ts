...
const callFromBody = (body: string) => { if (!body.startsWith("__CALL_INVITE__|")) return null; const [, id, mode, expiresAt] = body.split("|"); return id && (mode === "audio" || mode === "video") ? { id, mode, expiresAt: Number(expiresAt || 0) } : null; };
const visibleBody = (body: string) => { if (body.startsWith("__GURU__|")) return body.slice(9); if (body.startsWith("__EDITED__|")) return body.slice(11); const attachment = attachmentFromBody(body); if (attachment) return `[Attachment] ${attachment.name}`; const call = callFromBody(body); return call ? (call.mode === "video" ? "[Video call]" : "[Audio call]") : body; };
...
export async function GET(request: Request) {
  ...
  if (threadId) {
    ...
    const activeCall = await db.prepare("SELECT id, mode, status, started_by AS startedBy, expires_at AS expiresAt FROM message_calls WHERE thread_id = ? ORDER BY created_at DESC LIMIT 1").bind(threadId).first();
    const messages = messageRows.map(message => { const stored = String(message.body); return { ...message, guru: stored.startsWith("__GURU__|"), edited: stored.startsWith("__EDITED__|"), attachment: attachmentFromBody(stored), call: callFromBody(stored), body: stored.startsWith("__ATTACHMENT__|") || stored.startsWith("__CALL_INVITE__|") ? "" : visibleBody(stored) }; });
    return NextResponse.json({ thread, messages, members, currentUserId: user.id, activeCall });
  }
...
export async function POST(request: Request) {
  ...
  if (input.action === "edit_message") { const body = input.body?.trim() || ""; if (!input.threadId || !input.messageId || !body || body.length > 2000 || !await participant(input.threadId, user.id)) return NextResponse.json({ error: "Invalid edit" }, { status: 400 }); const editable = await db.prepare("SELECT id FROM messages WHERE id = ? AND thread_id = ? AND sender_id = ? AND deleted_at IS NULL AND body NOT LIKE '__ATTACHMENT__|%' AND body NOT LIKE '__GURU__|%' AND body NOT LIKE '__LIVE_REQUEST__|%' AND body NOT LIKE '__NOTICE_RESPONSE__|%' AND body NOT LIKE '__CALL_INVITE__|%'").bind(input.messageId, input.threadId, user.id).first(); if (!editable) return NextResponse.json({ error: "Message cannot be edited" }, { status: 403 }); await db.prepare("UPDATE messages SET body = ? WHERE id = ?").bind(`__EDITED__|${body}`, input.messageId).run(); await db.prepare("UPDATE message_threads SET updated_at = ? WHERE id = ?").bind(now, input.threadId).run(); return NextResponse.json({ ok: true }); }
  if (input.action === "add_member") { const memberId = String(input.memberId || ""); const targetThread = input.threadId ? await db.prepare("SELECT kind FROM message_threads WHERE id = ?").bind(input.threadId).first<{ kind: string }>() : null; if (targetThread?.kind === "class") return NextResponse.json({ error: "Course membership controls this room" }, { status: 403 }); if (!input.threadId || !memberId || memberId === user.id || !await participant(input.threadId, user.id) || !await db.prepare("SELECT id FROM users WHERE id = ?").bind(memberId).first()) return NextResponse.json({ error: "Choose a valid member" }, { status: 400 }); const existing = await db.prepare("SELECT id FROM message_participants WHERE thread_id = ? AND user_id = ?").bind(input.threadId, memberId).first<{ id: string }>(); if (existing) await db.prepare("UPDATE message_participants SET deleted_at = NULL, last_read_at = 0 WHERE id = ?").bind(existing.id).run(); else await db.prepare("INSERT INTO message_participants (id, thread_id, user_id, last_read_at) VALUES (?, ?, ?, 0)").bind(createId(), input.threadId, memberId).run(); await db.prepare("INSERT INTO messages (id, thread_id, sender_id, body, created_at) VALUES (?, ?, ?, ?, ?)").bind(createId(), input.threadId, user.id, `__MEMBER_JOINED__|${memberId}`, now).run(); await db.prepare("UPDATE message_threads SET kind = 'group', updated_at = ? WHERE id = ?").bind(now, input.threadId).run(); return NextResponse.json({ ok: true }); }
