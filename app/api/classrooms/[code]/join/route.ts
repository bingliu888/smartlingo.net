...
export async function POST(request:Request,{params}:{params:Promise<{code:string}>}){
  try{
    ...
    const playlistRelay=Boolean(body.playlistRelay&&room.realtimeMode!=="group_call"&&await db.prepare("SELECT 1 FROM live_class_playlist_state s JOIN live_class_playlist_items i ON i.room_id=s.room_id WHERE s.room_id=? AND s.active=1 LIMIT 1").bind(room.id).first());
