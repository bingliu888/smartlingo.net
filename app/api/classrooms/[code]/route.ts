...
export async function PATCH(request: Request, { params }: { params: Promise<{ code: string }> }) {
  ...
  const classType = courseRoom ? "private" : body.classType === "private" ? "private" : body.classType === "trial" ? "trial" : "public";
  const streamingMode = courseRoom ? "video" : body.streamingMode === "audio" ? "audio" : "video";
  const realtimeMode = courseRoom ? "webinar" : body.realtimeMode === "webinar" ? "webinar" : body.realtimeMode === "livestream" ? "livestream" : "group_call";
  ...
  return Response.json({ ok: true, courseWebinarLocked: Boolean(courseRoom) });
}
...
export async function DELETE(request: Request, { params }: { params: Promise<{ code: string }> }) {
  ...
  if (!await canManageClass(room, user)) return Response.json({ error: "Manager access required" }, { status: 403 });
  const database = getDatabase();
  if (await database.prepare("SELECT 1 FROM smartlingo_course_classrooms WHERE room_id=? LIMIT 1").bind(room.id).first()) {
    return Response.json({ error: "Course webinar rooms cannot be deleted" }, { status: 409 });
  }
