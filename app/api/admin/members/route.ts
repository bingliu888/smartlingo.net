...
export async function POST(request: Request) {
  ...
  if (body.action === "create") {
    ...
    if (name.length < 2 || password.length < 8) return Response.json({ error: "Name and an 8-character password are required" }, { status: 400 });
    if (await db.prepare("SELECT id FROM users WHERE lower(email)=? LIMIT 1").bind(email).first()) return Response.json({ error: "Member already exists" }, { status: 409 });
    ...
    if (!secretKey || !publishableKey) return Response.json({ error: "Identity service is not configured" }, { status: 503 });
    try {
      const clerk = await createClerkClient({ secretKey, publishableKey }).users.createUser({ emailAddress: [email], password, firstName: name });
      await db.prepare("INSERT INTO users(id,email,email_verified,display_name,password_hash,preferred_language,role,clerk_user_id,created_at) VALUES(?,?,0,?,?,?,'member',?,?)")
        .bind(clerk.id, email, name, `clerk$${await sha256(crypto.randomUUID())}`, "en", clerk.id, now)
        .run();
      await db.prepare("INSERT OR REPLACE INTO platform_member_access(user_id,status,subscriber_override,updated_by_user_id,created_at,updated_at) VALUES(?,'active',0,?,?,?)")
        .bind(clerk.id, admin.id, now, now)
        .run();
      return Response.json({ ok: true, id: clerk.id });
    } catch (error) {
      const status = typeof error === "object" && error && "status" in error ? Number((error as { status?: number }).status) : 500;
      return Response.json({ error: status === 422 ? "Email or password was rejected by the identity service" : "Unable to create member" }, { status: status === 422 ? 422 : 500 });
    }
  }

  const target = await db.prepare("SELECT id FROM users WHERE lower(email)=? LIMIT 1").bind(email).first<{ id: string }>();
  if (!target) return Response.json({ error: "Existing member not found" }, { status: 404 });
