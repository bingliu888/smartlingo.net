  ...
  try {
    ...
    if (input?.usdtPercent != null && !contractAddress) {
      ...
      if (!Number.isInteger(percent) || percent < 0 || percent > 100) {
        return NextResponse.json({ error: "SmartPay5 USDT percentage must be an integer from 0 to 100" }, { status: 400 });
      }
      const now = nowSeconds();
      await database().batch([
        database().prepare("UPDATE crypto_payment_settings SET smartpay5_usdt_percent=?,updated_at=? WHERE deleted_at IS NULL")
          .bind(percent, now),
        database().prepare(`INSERT INTO crypto_payment_admin_audit
          (id,admin_user_id,action,setting_id,created_at)
          VALUES (?,?,'update_smartpay5_usdt_percent',?,?)`)
          .bind(createId(), admin.id, settingId, now)
      ]);
      return NextResponse.json({ updated: true, usdtPercent: percent, scope: "site" });
    }
    if (!isAddress(contractAddress)) {
      return NextResponse.json({ error: "Enter a valid SmartPay5 address" }, { status: 400 });
    }
    ...
    const now = nowSeconds();
    await database().batch([
      database().prepare("UPDATE crypto_payment_settings SET smartpay5_contract=?,updated_at=? WHERE chain_id=? AND deleted_at IS NULL")
        .bind(contractAddress, now, setting.chainId),
      database().prepare(`INSERT INTO crypto_payment_admin_audit
        (id,admin_user_id,action,setting_id,created_at)
        VALUES (?,?,?,?,?)`)
        .bind(createId(), admin.id, "update_smartpay5_contract", settingId, now)
    ]);
