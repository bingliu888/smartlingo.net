...
export async function GET(request: Request) {
  try {
    ...
    if (!configuredContract || !isAddress(configuredContract)) {
      return NextResponse.json({ error: "SmartPay5 is not configured for this payment rail" }, { status: 409 });
    }
    ...
    const identity = await verifySmartPay5Identity(rpcUrl, contract);
    const [payouts, rules, latest] = await Promise.all([
      smartPay5PayoutConfigurationRaw(rpcUrl, contract),
      smartPay5PaymentRules(rpcUrl, contract),
      smartPay5LatestTransactions({ rpcUrl, contract, maxCount: 25 })
    ]);
