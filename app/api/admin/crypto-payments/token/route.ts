...
export async function GET(request: Request) {
  try {
    ...
    const rules = await smartPay5PaymentRules(rpcUrl, configuredContract as Address);
    ...
    if (!permittedTokens.includes(tokenAddress)) {
      return NextResponse.json({ error: "Select a token from an enabled SmartPay5 on-chain payment rule" }, { status: 422 });
    }
