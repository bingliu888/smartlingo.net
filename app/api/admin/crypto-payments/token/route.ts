...
import { smartPay4PaymentRules } from "../../../../../lib/smartpay4-server";
...
export async function GET(request: Request) {
  try {
    ...
    const rules = await smartPay4PaymentRules(rpcUrl, configuredContract as Address);
    ...
    if (!permittedTokens.includes(tokenAddress)) {
      return NextResponse.json({ error: "Select a token from an enabled SmartPay4 on-chain payment rule" }, { status: 422 });
    }
