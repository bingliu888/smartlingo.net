...
export default async function NewsPage({ params }: { params: Promise<{ lang: string }> }) {
  ...
  return <EditorialPage kind="news" lang={lang} editionDate={document.editionDate} eyebrow={zh ? "SMARTLINGO 动态" : "SMARTLINGO NEWS"} title={zh ? "了解语言路径、班级与学习社区的新进展。" : "See what is moving forward across language paths, classes, and Community."} intro={zh ? "关注听说读写训练、会员自主开班、人工智能导师、实时语音与公开项目计划的重要进展。" : "Follow meaningful progress across four-skill practice, member-led classes, the AI Guru, live audio, and the public roadmap."} cards={document[lang]}/>;
}
