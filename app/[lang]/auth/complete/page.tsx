...
export async function generateMetadata({ params }: { params: Promise<{ lang: string }> }): Promise<Metadata> {
  ...
  return { title: lang === "zh" || lang === "zh-tw" ? "正在完成登录" : "Completing sign-in" };
}
...
export default async function AuthCompletePage({
  ...
  return <main className="auth-page">
    <aside className="auth-art gc-auth-art smartlingo-auth-art">
      ...
      <blockquote>{lang === "zh" || lang === "zh-tw" ? "安全登录后，继续您的语言学习、课程与社区。" : "Continue your language learning, courses, and Community after secure sign-in."}</blockquote>
    </aside>
    <section className="auth-panel">
      <div className="auth-top">
        <Link href={`/${lang}`}>← {lang === "zh" || lang === "zh-tw" ? "返回首页" : "Back to home"}</Link>
