...
const pageCopy = {
  en: { title: "Sign in or join", description: "Use an email code, or choose a password. In password mode, a new email creates an account immediately without another email check.", quote: "Speak from day one. Learn with people.", back: "Back to home" },
  zh: { title: "登录或加入", description: "可使用邮箱验证码，或切换为密码。密码模式下，新邮箱会立即创建账户，无需再次验证邮箱。", quote: "从第一天开口，和真实的人一起学。", back: "返回首页" },
} as const;
...
export default async function AuthPage({ params, searchParams }: { params: Promise<{ lang: string; mode: string }>; searchParams: Promise<{ returnTo?: string }> }) {
  const { lang, mode } = await params; if (lang !== "en" && lang !== "zh" && lang !== "es" && lang !== "ja" && lang !== "ko" && lang !== "fr" && lang !== "de" && lang !== "ru" && lang !== "it" && lang !== "pt" && lang !== "ar" && lang !== "hi") notFound(); if (mode !== "login") redirect(`/${lang}/auth/login`); const t = pageCopy[lang === "zh" ? "zh" : "en"];
