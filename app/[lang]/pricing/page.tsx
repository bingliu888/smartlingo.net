...
export default async function PricingPage({ params }: { params: Promise<{ lang: string }> }) {
  ...
  return <main className="ai-cert-public-page lingo-public-page">
    ...
    <section className="lingo-pricing-grid">{SMARTLINGO_COURSE_PACKAGES.map((plan, index) => <article key={plan.tier} className={index === 1 ? "featured" : ""}><span>${plan.monthlyPriceCents / 100} / {zh ? "月" : "month"}</span><h2>{plan.name[lang]}</h2><p>{zh ? "第一个月免费；之后按月续订。" : "The first month is free; monthly renewal follows."}</p><ul>{plan.features[lang].map(item => <li key={item}>{item}</li>)}<li>{zh?"内置私密小组音视频与沟通中心":"Private group audio/video and communication center"}</li><li>{zh?"公开网络研讨会与 HLS 广播":"Public webinars and HLS broadcasting"}</li><li>{zh?"DeepSeek V4 Flash 默认，可选 OpenAI":"DeepSeek V4 Flash by default, OpenAI optional"}</li></ul><Link href={`/${lang}/programs`}>{zh ? "选择语言" : "Choose language"} →</Link><Link href={`/${lang}/pricing/crypto?plan=${plan.tier}`}>{zh?"连接钱包并使用加密货币付款":"Connect wallet · Pay with crypto"} →</Link></article>)}</section>
