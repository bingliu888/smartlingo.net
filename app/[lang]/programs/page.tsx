...
const copy = {
  en: {
    ...
    stages: [
      ...
      ["Focus", "Choose a real-life track", "Travel, workplace, hospitality, health support, customer service, study, or a teacher-created course focus."],
    ],
    ...
    practice: [
      ...
      ["Social learning", "Course goals, partner practice, Community topics, direct messages, and group Live Chat."],
    ],
    classKicker: "TEACHER OR COORDINATOR",
    classTitle: "Turn an approved path into your own learning community.",
    classBody: "Any signed-in member can prepare a private class. Choose the language and level, set a schedule and price, invite learners, coordinate daily work, and moderate the course Community. Public discovery and real payments require production review.",
    classOptions: [
      ["Use an approved path", "Open a course from a versioned SmartLingo curriculum without changing the controlled learning content."],
      ["Coordinate the course", "Add announcements, study goals, optional assignments, office hours, group chat, and progress reminders."],
  ...
  zh: {
    ...
    facts: [["目标语言", "12 种"], ["核心技能", "词汇 · 阅读 · 写作 · 听力 · 对话"], ["学习方式", "自主学习或课程带领"], ["反馈性质", "透明标注、只用于练习的人工智能建议"]],
    pathKicker: "学习路径",
    ...
    stages: [
      ...
      ["专项", "选择真实应用方向", "旅行、职场、酒店餐饮、健康支持、客户服务、留学，或老师创建的课程重点。"],
    ],
    ...
    practice: [
      ...
      ["共同学习", "课程目标、伙伴练习、社区主题、私信和群组实时聊天。"],
    ],
    classKicker: "老师或学习协调员",
    classTitle: "把经批准的学习路径变成自己的学习社区。",
    classBody: "任何已登录会员都可以准备专属课堂：选择语言与等级、设置日程和价格、邀请学员、协调每日任务并治理课程社区。进入公开目录和启用真实付款前必须完成生产审核。",
    classOptions: [
      ["使用经批准的路径", "从带版本的 SmartLingo 课程建立课程，不修改受控的核心学习内容。"],
      ["协调课程", "加入公告、学习目标、可选作业、答疑时间、群聊和进度提醒。"],
...
export default async function ProgramsPage({ params }: { params: Promise<{ lang: string }> }) {
  ...
  return <main className="ai-cert-public-page lingo-public-page" data-layout-page="programs">
    <div className="ai-public-hero-shell" data-layout-fill="programs-hero-shell"><SiteHeader lang={lang as any}/><section className="ai-public-hero"><p className="section-kicker">SMARTLINGO · {lang === "zh" ? "选择课程" : "CHOOSE COURSE"}</p><h1 data-layout-text-fit="programs-title">{t.title}</h1><p data-readable-copy="programs-hero-copy">{t.intro}</p><div className="ai-cert-actions" data-layout-track="programs-actions"><Link className="primary-button" href="#language-catalog">{t.start} →</Link><Link className="secondary-button" href={`/${lang}/assistant`}>{t.guide}</Link></div></section></div>
    <LearningPathPlanner lang={lang as any} catalogOnly/>
