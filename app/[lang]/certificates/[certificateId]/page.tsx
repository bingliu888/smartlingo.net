...
export default async function CertificateDetailPage({ params }: { params: Promise<{ lang: string; certificateId: string }> }) {
  ...
  return <main><SiteHeader lang={lang}/><div className="certificate-detail-page" data-layout-page="certificate-detail" data-layout-fill="certificate-detail-page" data-layout-ready="true">
    ...
    <article className="certificate-art" aria-label={zh ? "SmartLingo 结业证书" : "SmartLingo course certificate"}>
      ...
      <section><p>{zh ? "兹证明" : "This certifies that"}</p><h2>{certificate.memberName}</h2><p>{zh ? "已达到课程通过标准并完成" : "has met the passing standard and completed"}</p><h3>{certificateCourseName(certificate, (lang === "zh" || lang === "zh-tw") ? "zh" : "en")}</h3></section>
      <dl><div><dt>{zh ? "目标语言" : "Target language"}</dt><dd>{certificateLanguageName(certificate.targetLanguage, (lang === "zh" || lang === "zh-tw") ? "zh" : "en")}</dd></div><div><dt>{zh ? "课程等级" : "Course level"}</dt><dd>{certificate.level === "beginner" ? (zh ? "入门" : "Beginner") : certificate.level === "intermediate" ? (zh ? "中级" : "Intermediate") : (zh ? "高级" : "Advanced")}</dd></div><div><dt>{zh ? "课程长度" : "Course length"}</dt><dd>{certificate.durationDays} {zh ? "天" : "days"}</dd></div><div><dt>{zh ? "最终成绩" : "Final score"}</dt><dd>{certificate.finalScore} / 100</dd></div><div><dt>{zh ? "颁发日期" : "Issue date"}</dt><dd>{issueDate}</dd></div></dl>
