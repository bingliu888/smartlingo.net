...
export default async function CertificatesPage({ params }: { params: Promise<{ lang: string }> }) {
  ...
  const zh = lang === "zh";
  return <main><SiteHeader lang={lang}/><div className="certificate-page" data-layout-page="certificates" data-layout-fill="certificate-page" data-layout-ready="true">
    ...
    {certificates.length ? <section className="certificate-grid" aria-label={zh ? "证书列表" : "Certificate list"}>{certificates.map(certificate => <a className="certificate-tile" href={`/${lang}/certificates/${encodeURIComponent(certificate.id)}`} key={certificate.id}>
      <span className="certificate-tile-mark" aria-hidden="true">SL</span><div><p>{certificateLanguageName(certificate.targetLanguage, lang)} · {certificate.durationDays} {zh ? "天" : "days"}</p><h2>{certificateCourseName(certificate, lang)}</h2><strong>{certificate.finalScore}<small> / 100</small></strong><span>{new Date(certificate.issuedAt * 1000).toLocaleDateString(zh ? "zh-CN" : "en-US")}</span></div>
    </a>)}</section> : <div className="certificate-empty"><h2>{zh ? "完成第一门课程后，证书会显示在这里。" : "Your first certificate will appear here after you pass a course."}</h2><a href={`/${lang}/classes`}>{zh ? "浏览语言课程" : "Browse language classes"} →</a></div>}
