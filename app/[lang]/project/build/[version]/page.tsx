...
export default async function BuildReportPage({ params }: { params: Promise<{ lang: string; version: string }> }) {
  ...
  return <main className="gg-project-page"><SiteHeader lang={lang}/><article className="gg-detail">
    ...
    <h1>{build.title[(lang === "zh" || lang === "zh-tw") ? "zh" : "en"]}</h1>
    <div className="gg-detail-summary"><span>{zh ? "已部署" : "DEPLOYED"}</span><b>{zh ? "提交" : "commit"} {build.commit}</b></div>
    <section><h2>{zh ? "本次构建完成" : "Completed in this build"}</h2><ul>{build.completed[(lang === "zh" || lang === "zh-tw") ? "zh" : "en"].map(item => <li key={item}>{item}</li>)}</ul></section>
    <section><h2>{zh ? "可测试内容" : "Ready to test"}</h2><ul>{build.testable[(lang === "zh" || lang === "zh-tw") ? "zh" : "en"].map(item => <li key={item}>{item}</li>)}</ul></section>
