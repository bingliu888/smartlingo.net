...
export default async function ReportPage({ params }: { params: Promise<{ lang: string; date: string }> }) {
  ...
  if (!report) notFound();
  const zh = lang === "zh";
  return <main className="gg-project-page"><SiteHeader lang={lang}/><article className="gg-detail"><Link href={`/${lang}/project`}>← {zh ? "项目日历" : "Project calendar"}</Link><p className="section-kicker">{zh ? "每日报告" : "DAILY REPORT"} · {date}</p><h1>{report.title[lang]}</h1><div className="gg-detail-summary"><span>{report.beta[lang]}</span><b>{report.completed} {zh ? "项里程碑" : "milestones"}</b></div><section><h2>{zh ? "今日完成" : "Completed today"}</h2><p>{report.summary[lang]}</p></section><section><h2>{zh ? "验证记录" : "Validation record"}</h2><ul>{report.validation[lang].map(item => <li key={item}>{item}</li>)}</ul></section><section><h2>{zh ? "回滚边界" : "Rollback boundary"}</h2><p>{report.rollback[lang]}</p></section><section><h2>{zh ? "下一日计划" : "Next delivery day"}</h2><p>{report.next[lang]}</p></section><Link className="gg-detail-link" href={`/${lang}/project`}>{zh ? "返回项目日历" : "Back to project calendar"} →</Link></article><SiteFooter lang={lang}/></main>;
}
