...
export default async function SmartCardTutorial({ params }: { params: Promise<{ lang: string }> }) {
  ...
  const steps = zh ? [
    ["先学", "先看含义，自己回想目标语言，再翻卡核对；播放发音并跟读，不要只浏览答案。"],
    ["间隔后再想", "离开几分钟或等到第二天再挑战。真正的记忆来自隔开后的提取，而不是短时间连续点击。"],
    ["完成混合挑战", "题目包含词义、听辨、主动回忆和少量书写；速度不影响积分，达到 80 分即合格。"],
    ["分享给朋友", "复制公开链接。朋友无需登录即可学习；自己挑战自己的卡不会获得可抵费积分。"],
    ["安全领取", "访客积分只保存在本设备。登录后领取到独立课程积分账本；不会自动订阅或开始免费月。"],
    ["兑换一个月", "100 点抵 1 美元。余额达到课程当月价格后，可一次兑换 30 天，不自动续订。"],
  ] : [
    ["Learn first", "Read the meaning, recall the target language before flipping, then check. Play the model and repeat; do not only browse answers."],
    ...
    ["Take a mixed challenge", "Questions combine meaning, listening, active recall, and limited typing. Speed never affects points; 80 is a pass."],
    ...
    ["Claim safely", "Guest points remain on this device. Sign in to claim them into the separate course-credit ledger; no subscription or free month starts."],
