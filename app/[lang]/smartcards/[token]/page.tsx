...
export default async function PublicSmartCardPage({ params, searchParams }: { params: Promise<{ lang: string; token: string }>; searchParams: Promise<{ mode?: string; day?: string }> }) {
  const { lang, token } = await params; if (lang !== "en" && lang !== "zh" && lang !== "zh-tw" && lang !== "es" && lang !== "ja" && lang !== "ko" && lang !== "fr" && lang !== "de" && lang !== "ru" && lang !== "it" && lang !== "pt" && lang !== "ar" && lang !== "hi") notFound();
  ...
  return <PublicSmartCardChallenge lang={(lang === "zh" || lang === "zh-tw") ? "zh" : "en"} token={token} dayNumber={query.day ? dayNumber : undefined} gameMode={query.mode === "challenge" ? "challenge" : "practice"}/>;
}
