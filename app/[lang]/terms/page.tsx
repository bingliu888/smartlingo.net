...
const copy = {
  en: {
    ...
    sections: [
      ...
      ["Member-led courses", "Every signed-in member may prepare a private course as teacher or coordinator using an approved language path. Public listing, higher-risk content, and production payment require applicable review. Course owners are responsible for truthful descriptions, lawful conduct, learner support, and their tax obligations."],
      ["Course economics", "The planned first successful payment by a learner in each course receives one 15% discount. The actual discounted pre-tax amount is split 70% to the course owner and 30% to the platform. Taxes, processing fees, refunds, and disputes are handled separately and shown in the ledger."],
      ...
      ["SmartCards and course credit", "Public SmartCards may be used without an account. Guest points are provisional until claimed after sign-in. Only a first server-scored pass for a published deck version may earn course credit, subject to self-challenge, daily-cap, fraud, and eligibility controls. Course credit has no cash or transfer value and can be used only for eligible SmartLingo course periods."],
  ...
  zh: {
    ...
    sections: [
      ...
      ["会员自主开班", "每位登录会员都可使用经批准的语言路径，以老师或协调员身份准备专属课堂。进入公开目录、高风险内容与生产收费须完成适用审核。班主应对真实描述、合法运营、学员支持与自身税务义务负责。"],
      ["课程分账", "每位学员在每个课程的首次成功付款计划享一次八五折；以实际折后税前金额为基础，班主获得七成，平台获得三成。税款、处理费、退款和争议另行处理并显示在账本中。"],
      ["平台订阅与奖励", "平台订阅和会员创建课程的付款彼此独立。验证成功的平台订阅收费可按公布规则为一位直接介绍人产生积分；注册、课程结账、班主收款、退款、争议、打赏和连接账户收费永不产生这些积分。"],
      ["SmartCard 与课程积分", "公开 SmartCard 可在没有账户时使用；访客积分在登录领取前只是待领取记录。只有已发布词卡版本的首次服务器合格评分，才可能在自我挑战、每日上限、防欺诈与资格规则下产生课程积分。课程积分不可兑现或转让，只能用于符合条件的 SmartLingo 课程周期。"],
...
export default async function TermsPage({ params }: { params: Promise<{ lang: string }> }) { const { lang } = await params; if (lang !== "en" && lang !== "zh" && lang !== "es" && lang !== "ja" && lang !== "ko" && lang !== "fr" && lang !== "de" && lang !== "ru" && lang !== "it" && lang !== "pt" && lang !== "ar" && lang !== "hi") notFound(); const t = copy[lang === "zh" ? "zh" : "en"]; return <main className="ai-cert-legal-page lingo-public-page"><SiteHeader lang={lang as any}/><article className="ai-cert-legal-main"><div className="ai-draft-note"><strong>{lang === "zh" ? "草案 · 待正式法律审核" : "DRAFT · FORMAL LEGAL REVIEW PENDING"}</strong><span>{t.intro}</span></div><p className="section-kicker">{lang === "zh" ? "使用条款" : "TERMS OF USE"}</p><h1>{t.title}</h1><div className="ai-legal-sections">{t.sections.map(([title, body]) => <section key={title}><h2>{title}</h2><p>{body}</p></section>)}</div></article><SiteFooter lang={lang as any}/></main>; }
