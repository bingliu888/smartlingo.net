...
export default async function CourseTrialSkillPage({ params }: { params: Promise<{ lang: string; classId: string; skill: string }> }) {
  ...
  if ((lang !== "en" && lang !== "zh" && lang !== "zh-tw" && lang !== "es" && lang !== "ja" && lang !== "ko" && lang !== "fr" && lang !== "de" && lang !== "ru" && lang !== "it" && lang !== "pt" && lang !== "ar" && lang !== "hi") || !isSmartLingoCommunityLanguage(language) || !SKILLS.includes(skill as Skill)) notFound();
  ...
  const tasks = taskSkills.map(taskSkill => buildDailyPracticeItem(language, taskSkill, date, (lang === "zh" || lang === "zh-tw") ? "zh" : "en", "beginner"));
  return <main className="learning-page" data-layout-page="anonymous-course-trial"><SiteHeader lang={lang}/><AnonymousBeginnerTrial lang={(lang === "zh" || lang === "zh-tw") ? "zh" : "en"} language={language} languageName={`${item.nativeName} · ${lang === "zh" ? item.nameZh : item.nameEn}`} speechLocale={item.speechLocale} direction={item.direction} cards={cards} tasks={tasks} initialSkill={skill as Skill} lockedSkill classId={classId}/><SiteFooter lang={lang}/></main>;
}
