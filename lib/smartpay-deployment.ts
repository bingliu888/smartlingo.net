...
export type SmartPay3DeploymentArtifact = {
...
type SmartPay3DeploymentProvider = {
  request(args: { method: string; params?: unknown[] }): Promise<unknown>;
};

const DEPLOYMENT_GAS_BUFFER_PERCENT = 25n;
export const SMARTPAY3_FACTORY_ADDRESS = "0xce0042B868300000d44A59004Da54A005ffdcf9f" as Address;
export const SMARTPAY3_FACTORY_RUNTIME_CODE_HASH = "0xc4d5542b53a8b779595a20a8ddd60e58a6c49d3c3decc2df83ced1c69c8ca807" as Hex;

const SMARTPAY3_FACTORY_ABI = [{
  type: "function",
  name: "deploy",
...
export function smartPay3DeploymentData(artifact: SmartPay3DeploymentArtifact, initialOwner: Address, abi: Abi) {
  if (artifact.contractName !== "SmartPay3" || artifact.constructorInputs?.[0] !== "initialOwner"
    || !/^0x(?:[0-9a-f]{2})+$/i.test(artifact.bytecode) || !isAddress(initialOwner)) {
    throw new Error("INVALID_SMARTPAY3_DEPLOYMENT_ARTIFACT");
  }
  return encodeDeployData({ abi, bytecode: artifact.bytecode, args: [initialOwner] });
}

export function smartPay3AddressFromReceipt(receipt: { contractAddress?: unknown; to?: unknown }, expectedFactoryAddress?: unknown) {
  ...
  if (isAddress(expected) && String(receipt.to || "").toLowerCase() === SMARTPAY3_FACTORY_ADDRESS.toLowerCase()) {
    return expected as Address;
  }
  throw new Error("DEPLOYMENT_ADDRESS_MISSING");
}

export function smartPay3FactoryDeploymentFromSalt(creationData: Hex, salt: Hex) {
  if (!/^0x(?:[0-9a-f]{2})+$/i.test(creationData) || !/^0x[0-9a-f]{64}$/i.test(salt)) {
    throw new Error("INVALID_SMARTPAY3_FACTORY_DEPLOYMENT");
  }
  return {
    factoryAddress: SMARTPAY3_FACTORY_ADDRESS,
    salt,
    contractAddress: getCreate2Address({ from: SMARTPAY3_FACTORY_ADDRESS, salt, bytecode: creationData }),
    data: encodeFunctionData({ abi: SMARTPAY3_FACTORY_ABI, functionName: "deploy", args: [creationData, salt] })
  };
}

export function smartPay3FactoryDeployment(
  creationData: Hex,
  ...
  const salt = keccak256(stringToHex(deploymentId
    ? `smartpay3:v2:${siteScope.toLowerCase()}:${chainId}:${initialOwner.toLowerCase()}:${deploymentId.toLowerCase()}`
    : `smartpay3:v1:${siteScope.toLowerCase()}:${chainId}:${initialOwner.toLowerCase()}`));
  return smartPay3FactoryDeploymentFromSalt(creationData, salt);
}

export function smartPay3DeploymentGasLimit(estimate: unknown) {
...
export async function requestSmartPay3Deployment(
  provider: SmartPay3DeploymentProvider,
  from: Address,
  creationData: Hex,
  input: { chainId: number; siteScope: string; deploymentId?: string }
) {
  if (!isAddress(from) || !/^0x(?:[0-9a-f]{2})+$/i.test(creationData)) {
    throw new Error("INVALID_SMARTPAY3_DEPLOYMENT_TRANSACTION");
  }
  const deployment = smartPay3FactoryDeployment(creationData, from, input.chainId, input.siteScope, input.deploymentId);
  ...
  if (typeof factoryCode !== "string" || !/^0x(?:[0-9a-f]{2})+$/i.test(factoryCode)
    || keccak256(factoryCode as Hex) !== SMARTPAY3_FACTORY_RUNTIME_CODE_HASH) {
  ...
  const transaction = { ...transactionBase, gas: smartPay3DeploymentGasLimit(estimate) };
