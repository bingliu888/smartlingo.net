...
import { SMARTPAY4_ABI, SMARTPAY4_TRANSACTION_RECORDED_TOPIC } from "./smartpay4";
import { locateSmartPay4Receipt } from "./smartpay4-receipt-locator";
...
async function ethCall(rpcUrl: string, contract: Address, functionName: string, args: readonly unknown[] = []) {
  const data = encodeFunctionData({ abi: SMARTPAY4_ABI, functionName, args });
...
export async function smartPay4TransactionById(rpcUrl: string, contract: Address, transactionId: Hex) {
  const data = await ethCall(rpcUrl, contract, "transactionById", [transactionId]);
  const record = decodeFunctionResult({ abi: SMARTPAY4_ABI, functionName: "transactionById", data }) as ContractTransactionRecord;
  return mapTransactions([record])[0];
}

export async function smartPay4ReceiptByTransactionId(input: {
  rpcUrl: string;
  contract: Address;
  transactionId: Hex;
  timestamp: number;
}) {
  return locateSmartPay4Receipt({
    rpc: <T>(method: string, params: unknown[]) =>
      cryptoRpc<T>(input.rpcUrl, method, params),
    contract: input.contract,
    transactionId: input.transactionId,
    transactionTopic: SMARTPAY4_TRANSACTION_RECORDED_TOPIC,
    timestamp: input.timestamp,
  });
}

export async function smartPay4PayoutConfigurationRaw(rpcUrl: string, contract: Address) {
...
export async function smartPay4PaymentRules(rpcUrl: string, contract: Address) {
  const countData = await ethCall(rpcUrl, contract, "paymentRuleCount");
  const count = Number(decodeFunctionResult({ abi: SMARTPAY4_ABI, functionName: "paymentRuleCount", data: countData }) as bigint);
  ...
  for (let offset = 0; offset < count; offset += 100) {
    ...
    records.push(...decodeFunctionResult({ abi: SMARTPAY4_ABI, functionName: "paymentRules", data: pageData }) as ContractPaymentRule[]);
  }
  return records.map(record => ({
    primaryTokenAddress: record.primaryTokenAddress,
    secondaryTokenAddress: record.secondaryTokenAddress,
...
export async function smartPay4LatestTransactions(input: {
  rpcUrl: string;
  contract: Address;
  payerId?: string;
  maxCount: number;
  ...
  if (!Number.isInteger(input.maxCount) || input.maxCount < 1 || input.maxCount > 100) {
    throw new Error("INVALID_SMARTPAY4_LATEST_COUNT");
  }
...
export async function smartPay4Ownership(rpcUrl: string, contract: Address) {
  const [ownerData, pausedData] = await Promise.all([
    ethCall(rpcUrl, contract, "owner"),
    ethCall(rpcUrl, contract, "paused")
  ]);
  return {
    owner: decodeFunctionResult({ abi: SMARTPAY4_ABI, functionName: "owner", data: ownerData }) as Address,
    paused: decodeFunctionResult({ abi: SMARTPAY4_ABI, functionName: "paused", data: pausedData }) as boolean
  };
}

export async function verifySmartPay4Identity(rpcUrl: string, contract: Address) {
  ...
  const expectedCode = String((smartPay4ArtifactJson as { deployedBytecode?: string }).deployedBytecode || "");
  ...
  const [ownership, basicData, intermediateData, advancedData] = await Promise.all([
    smartPay4Ownership(rpcUrl, contract),
  ...
  const ids = [
    decodeFunctionResult({ abi: SMARTPAY4_ABI, functionName: "MAIN_ID_BASIC_3_MONTH", data: basicData }) as string,
    decodeFunctionResult({ abi: SMARTPAY4_ABI, functionName: "MAIN_ID_INTERMEDIATE_3_MONTH", data: intermediateData }) as string,
    decodeFunctionResult({ abi: SMARTPAY4_ABI, functionName: "MAIN_ID_ADVANCED_3_MONTH", data: advancedData }) as string
  ];
