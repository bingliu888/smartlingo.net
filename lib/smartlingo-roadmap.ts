...
const smartLingoRoadmapJson = String.raw`[
  {
    ...
    "tasks": [
      ...
      { "id": "sl-d01-commercial-rules", "status": "done", "progress": 100, "title": { "zh": "固定班级收费与积分规则", "en": "Freeze class-commerce and reward rules" }, "summary": { "zh": "任何已登录会员都可创建专属课堂；班级首笔付款优惠百分之十五，优惠后税前金额按班主百分之七十、平台百分之三十分配；班级付款永不产生推荐积分。", "en": "Any signed-in member may create a private class; the first class payment is 15% off, and the discounted pre-tax amount splits 70% to the owner and 30% to the platform; class payments never create referral points." } },
  ...
  },
  {
    ...
    "tasks": [
      { "id": "sl-d02-clerk-auth", "status": "done", "progress": 100, "title": { "zh": "接入 Clerk 邮箱验证码身份", "en": "Integrate Clerk email-code identity" }, "summary": { "zh": "生产 Clerk 已完成无密码首次注册、邮箱验证码、会话桥接、独立域名与 SSL；首位用户及四名测试用户已通过 Safari 真实登录，受保护路由、推荐 Cookie 与应用会话均在正式域名验收。", "en": "Production Clerk now supports passwordless first registration, email codes, the session bridge, its dedicated domain, and SSL; the first user and four test users completed real Safari sign-in, and protected routes, the referral cookie, and app sessions were accepted on the production domain." } },
      { "id": "sl-d02-d1-model", "status": "done", "progress": 100, "title": { "zh": "建立 D1 核心数据模型", "en": "Build the D1 core data model" }, "summary": { "zh": "保存学习路径、原创双语练习、版本化进度、班级、成员、精确分账订单、平台订阅付款、单层直接推荐与幂等积分账本，并由迁移约束拒绝伪造关系。", "en": "Store learning paths, original bilingual exercises, versioned progress, classes, members, exact-split orders, platform subscription payments, single-level direct referrals, and an idempotent reward ledger, with migration constraints rejecting forged relationships." } },
      { "id": "sl-d02-r2-media", "status": "done", "progress": 100, "title": { "zh": "建立 R2 私有媒体存储", "en": "Establish private R2 media storage" }, "summary": { "zh": "为头像、班级封面、语音练习和聊天附件建立内容签名、大小限制、D1 范围绑定、授权读取与可重试删除策略。", "en": "Add content signatures, size limits, D1 scope binding, authorized reads, and retry-safe deletion policies for avatars, class covers, voice practice, and chat attachments." } },
      { "id": "sl-d02-openai-gateway", "status": "done", "progress": 100, "title": { "zh": "建立智能服务网关", "en": "Build the AI service gateway" }, "summary": { "zh": "统一文字智能导师、听力、口语、写作反馈与安全审核的服务端策略，加入键控主体哈希、限流、用量审计、完整响应超时和安全回退。", "en": "Centralize server-side policies for text Guru, listening, speaking, writing feedback, and safety review with keyed subject hashes, limits, usage audits, full-response timeouts, and safe fallbacks." } },
      { "id": "sl-d02-infrastructure-tests", "status": "done", "progress": 100, "title": { "zh": "测试基础设施隔离", "en": "Test infrastructure isolation" }, "summary": { "zh": "验证迁移重复执行、订单与积分伪造拒绝、媒体越权与上传限制、前端产物密钥扫描，以及身份或智能服务故障的可恢复状态。", "en": "Verify repeatable migrations, rejected order and reward forgery, denied media access and constrained uploads, client-artifact secret scanning, and recoverable identity or AI failures." } }
    ]
  },
  {
    "date": "2026-08-02",
    "status": "done",
    "category": { "zh": "十二种语言与学习路径", "en": "Twelve languages and learning paths" },
    "owner": { "zh": "SmartLingo 课程团队", "en": "SmartLingo curriculum team" },
    "acceptance": { "zh": "学习者可从中文、英语、西班牙语、日语、韩语、法语、德语、俄语、意大利语、葡萄牙语、阿拉伯语和印地语选择清晰、版本化的学习路径。", "en": "Learners can choose clear, versioned paths for Chinese, English, Spanish, Japanese, Korean, French, German, Russian, Italian, Portuguese, Arabic, and Hindi." },
    "tasks": [
      { "id": "sl-d03-language-catalog", "status": "done", "progress": 100, "title": { "zh": "发布十二种语言目录", "en": "Publish the twelve-language catalog" }, "summary": { "zh": "十二种语言现有稳定编号、双语与本名、文字方向、设备相关语音能力、三阶段编号、原创来源和内容版本；目录只用文字，不以国旗代替语言。", "en": "All twelve languages now have stable IDs, bilingual and native names, writing direction, device-dependent speech capabilities, three stage IDs, original provenance, and a content version; the catalog is text-only and never substitutes flags for languages." } },
      { "id": "sl-d03-goal-onboarding", "status": "done", "progress": 100, "title": { "zh": "建立目标与水平引导", "en": "Build goal and level onboarding" }, "summary": { "zh": "公开课程页可选择目标语言、使用场景、每日 5 至 20 分钟、自报水平与自适应、自选或基础起步方式；登录后由 D1 持久保存单一有效计划。", "en": "The public Programs page now captures target language, use case, a 5–20 minute daily goal, self-reported level, and adaptive, self-selected, or fundamentals entry; signed-in members persist one active plan in D1." } },
      { "id": "sl-d03-placement-check", "status": "done", "progress": 100, "title": { "zh": "建立入班自适应分级", "en": "Build class-entry adaptive placement" }, "summary": { "zh": "入班流程支持自选初中高级，或使用版本化原创双语题完成可暂停、可重测、可跳过的十五题五技能测评；跳过记录为空分并明确只推荐练习起点，不是正式证书。", "en": "Class entry now supports self-selected beginner, intermediate, or advanced placement, or a pausable, retakeable, skippable 15-item five-skill check with versioned original bilingual items; skips store null scores and recommendations are practice starting points, never credentials." } },
      { "id": "sl-d03-unit-map", "status": "done", "progress": 100, "title": { "zh": "建立阶段与单元地图", "en": "Build the stage and unit map" }, "summary": { "zh": "每条语言路径现有基础、日常与独立三个阶段、九个原创双语单元和显式线性先决条件，覆盖词汇、阅读、写作、听力、对话与真实任务，并清楚区分可用与预览内容。", "en": "Every language path now has Foundation, Everyday, and Independent stages, nine original bilingual units, and explicit linear prerequisites spanning vocabulary, reading, writing, listening, dialogue, and real tasks, with available and preview content clearly distinguished." } },
      { "id": "sl-d03-path-tests", "status": "done", "progress": 100, "title": { "zh": "测试学习路径完整性", "en": "Test learning-path integrity" }, "summary": { "zh": "单元、API、D1 迁移和运行时版面回归覆盖十二语切换、阶段边界、内容版本、定位重测与跳过、进度保留、非法路径或单元拒绝、无内容双语回退及五个发布视口。", "en": "Unit, API, D1 migration, and runtime layout regressions cover twelve-language switching, stage boundaries, content versions, placement retakes and skips, progress retention, invalid path or unit rejection, bilingual no-content fallback, and all five release viewports." } }
    ]
  },
  {
    "date": "2026-08-03",
    ...
    "tasks": [
      { "id": "sl-d04-session-composer", "status": "done", "progress": 100, "title": { "zh": "建立每日练习编排", "en": "Build the daily session composer" }, "summary": { "zh": "服务端现按会员目标、阶段与单元、近三十日五技能分和到期词汇，确定性编排固定六十分钟的原创新课、间隔复习、五技能练习与结束回顾；模板重点不再隐藏阅读或写作。", "en": "The server now deterministically composes a fixed 60-minute loop of original new material, spaced review, all five skills, and a recap from the member goal, stage and unit, recent 30-day skill scores, and due vocabulary; template focus no longer hides reading or writing." } },
      { "id": "sl-d04-answer-feedback", "status": "done", "progress": 100, "title": { "zh": "实现即时反馈与讲解", "en": "Implement instant feedback and explanations" }, "summary": { "zh": "服务器重建版本化题目并判分，普通练习与测验逐题保存并显示正确性、原创双语讲解、提示和内容版本；跳过保持为跳过，反馈明确不是真人教师或官方考试。", "en": "The server reconstructs and grades versioned items, then stores and shows correctness, original bilingual explanations, hints, and content versions for practice and each quiz response; skips remain skips and feedback is explicitly neither a human teacher nor an official exam." } },
      { "id": "sl-d04-learning-xp", "status": "done", "progress": 100, "title": { "zh": "建立学习经验值与连续学习", "en": "Build learning XP and streaks" }, "summary": { "zh": "学习 XP 只由成功写入的服务器活动幂等产生，使用独立无现金价值账本；连续学习锁定权威 IANA 时区并用修订号 CAS 收敛并发，每滚动三十天最多公平修复一个单日空档。", "en": "Learning XP is idempotently derived only from saved server activities in a separate no-cash-value ledger; streaks lock an authoritative IANA timezone and converge concurrent reconciliation through revision CAS, with at most one fair single-day repair per rolling 30 days." } },
      { "id": "sl-d04-resume-sync", "status": "done", "progress": 100, "title": { "zh": "实现中断恢复与跨设备同步", "en": "Implement resume and cross-device sync" }, "summary": { "zh": "课程日现有不可变编排、服务器修订快照、原子操作回执、三方草稿合并和比较更新；弱网草稿只暂存于会话，冲突保留两边，课程完成只信任服务器五技能、测验与计时证据。", "en": "Course days now have immutable plans, server revision snapshots, atomic operation receipts, three-way draft merge, and compare-and-swap updates; weak-network drafts stay session-scoped, conflicts preserve both sides, and completion trusts only server five-skill, quiz, and timer evidence." } },
      { "id": "sl-d04-loop-tests", "status": "done", "progress": 100, "title": { "zh": "测试每日学习循环", "en": "Test the daily learning loop" }, "summary": { "zh": "纯函数、API 合约、D1 原子测验与迁移、断线恢复、重复测验、跨月与时区、并发 CAS、错误讲解、键盘标签和五视口双语运行时版面均进入回归门槛。", "en": "Pure functions, API contracts, atomic D1 quizzes and migrations, offline recovery, duplicate quizzes, month and timezone boundaries, concurrent CAS, incorrect-answer explanations, keyboard tabs, and bilingual five-viewport runtime layouts are now release regressions." } }
    ]
  },
  {
    "date": "2026-08-04",
    ...
    "acceptance": { "zh": "有来源、可审计的词汇内容通过卡片、语境和间隔复习形成可解释、可调整的长期记忆计划。", "en": "Sourced, auditable vocabulary content supports an explainable, adjustable long-term memory plan through cards, context, and spaced review." },
    "tasks": [
      { "id": "sl-d05-vocabulary-records", "status": "done", "progress": 100, "title": { "zh": "建立版本化词汇记录", "en": "Build versioned vocabulary records" }, "summary": { "zh": "十二语词汇以稳定编号和不可变版本保存词形、标准音标、双语词义、难度和主题；每条发布数据同时记录词频、词典、许可、版本和真实审核方式，不把自动校验误称为人工审核。", "en": "Twelve-language vocabulary uses stable IDs and immutable versions for forms, standard phonetics, bilingual meanings, difficulty, and topics; every published row records its frequency, dictionary, license, revision, and truthful review method without presenting automated checks as human review." } },
  ...
  },
  {
    ...
    "tasks": [
      ...
      { "id": "sl-d10-private-access", "status": "planned", "progress": 0, "title": { "zh": "实现专属课堂访问控制", "en": "Implement private-class access control" }, "summary": { "zh": "只有班主、受邀者和已报名成员可见班级内容与名册；邮箱和付款资料不对成员公开。", "en": "Only owners, invitees, and enrolled members can see class content and rosters; email and payment details stay private." } },
  ...
  },
  {
    ...
    "acceptance": { "zh": "专属课堂可通过邀请增长；只有通过内容、价格、班主和安全审核的版本才能进入公开目录。", "en": "Private classes grow by invitation; only versions passing content, price, owner, and safety review can enter the public catalog." },
    "tasks": [
  ...
  },
  {
    ...
    "tasks": [
      ...
      { "id": "sl-d18-responsive-contract", "status": "planned", "progress": 0, "title": { "zh": "执行最高优先级自适应版面合约", "en": "Enforce the highest-priority responsive layout contract" }, "summary": { "zh": "在 390×844、430×932、834×1112、1194×834 和 1440×1000 的中英文页面实测文字容纳、面板宽度、单栏语义组和最终计算样式。", "en": "Measure text containment, panel width, single-column semantic groups, and final computed styles in both languages at 390×844, 430×932, 834×1112, 1194×834, and 1440×1000." } },
  ...
  },
  {
    ...
    "tasks": [
      ...
      { "id": "sl-d19-learning-journey", "status": "planned", "progress": 0, "title": { "zh": "验证完整语言学习流程", "en": "Verify the full language-learning journey" }, "summary": { "zh": "完成目标选择、入班分级、每日词汇阅读写作听力对话、学习日历、社区活动、智能导师、实时语音、进度同步与隐私删除。", "en": "Complete target choice, class placement, daily vocabulary/reading/writing/listening/dialogue, learning calendar, Community activity, Guru, Live Audio, progress sync, and privacy deletion." } },
      { "id": "sl-d19-class-social-journey", "status": "planned", "progress": 0, "title": { "zh": "验证开班与共同学习", "en": "Verify class creation and social learning" }, "summary": { "zh": "让测试 1 作为教师或协调员建专属课堂，邀请测试 2、3、4，并验证论坛、共同目标、私信和群组聊天。", "en": "Have test one create a private class as teacher or coordinator, invite tests 2–4, and verify forums, shared goals, direct messages, and group chat." } },
