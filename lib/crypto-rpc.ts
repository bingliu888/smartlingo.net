...
export const addressTopic = (address: string) =>
...
export async function cryptoRpcUrl(chainId: number) {
  ...
  return values[`CRYPTO_RPC_URL_${chainId}`]
    || values.CRYPTO_RPC_URL
    || publicRpc[chainId]
    || "";
}
...
export async function cryptoRpc<T = unknown>(
  ...
  const response = await withExternalRequestTimeout((signal) => fetch(url, {
    method: "POST",
  ...
  if (!raw.truncated) {
    try {
      payload = JSON.parse(raw.text) as { result?: T; error?: { message?: string } };
    } catch { /* invalid RPC JSON */ }
  }
  if (!response.ok || payload.error || payload.result == null)
    throw new Error(payload.error?.message || "RPC_UNAVAILABLE");
